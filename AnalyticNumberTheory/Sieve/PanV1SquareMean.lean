...
lemma lcm_inv_sum_le (N : ℕ) :
    (∑ d ∈ Finset.Icc 1 N, ∑ e ∈ Finset.Icc 1 N, (1 : ℝ) / (Nat.lcm d e : ℝ)) ≤
      (1 + Real.log (N + 1)) ^ 3 := by
  have hQ : Finset.Icc 1 N ⊆ Finset.Ioc 0 ⌊(N : ℝ) + 1⌋₊ := by
    intro q hq
    obtain ⟨hq1, hqN⟩ := Finset.mem_Icc.mp hq
    apply Finset.mem_Ioc.mpr
    refine ⟨hq1, ?_⟩
    exact hqN.trans (Nat.le_floor (by linarith : (N : ℝ) ≤ (N : ℝ) + 1))
  have hc : ∀ q ∈ Finset.Icc 1 N, |(1 : ℝ)| ≤
      (MathlibNt.AnalyticNumberTheory.LargeSieve.LiLiuPrereqFouvry.fouvryTau 1 q : ℝ) := by
    intro q hq
    rw [MathlibNt.AnalyticNumberTheory.LargeSieve.LiLiuPrereqFouvry.fouvryTau_order_one
      (by have := (Finset.mem_Icc.mp hq).1; omega)]
    norm_num
  have h := MathlibNt.AnalyticNumberTheory.LargeSieve.LcmWeightBounds.sum_abs_lcm_weight_le_log
    1 (L := (N : ℝ) + 1) (by have := Nat.cast_nonneg (α := ℝ) N; linarith)
    (Finset.Icc 1 N) hQ (fun _ => 1) hc
  simpa only [one_mul, abs_div, abs_one, Nat.abs_cast, one_pow, mul_one,
    show (2 : ℕ) + 1 = 3 from rfl] using h
