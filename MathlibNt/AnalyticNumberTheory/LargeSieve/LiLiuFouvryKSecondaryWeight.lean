import MathlibNt.AnalyticNumberTheory.LargeSieve.LiLiuFouvryKSecondaryGeometry
...
theorem directPayKSecondary_weight_paid
    {Cscale κ δ ρ C Csec Ccoeff : ℝ} (hscale : 1 ≤ Cscale)
    (hκ : 0 ≤ κ) (hδ : 0 < δ) (hC : 0 ≤ C) (hCsec : 0 ≤ Csec) :
    ∃ Cw : ℝ, 0 < Cw ∧ ∀ (x : ℝ), 1 ≤ x → ∀ (a : ℤ)
      (K : WExtractedKey) (F : ℕ) (j : Fin 5 → ℕ),
      |(a : ℝ)| ≤ Cscale * x → (2 : ℝ) ^ j 0 ≤ 8 * x ^ (6 : ℕ) →
  ...
  obtain ⟨Ca, hCa, htau⟩ := directPayKSecondary_tau hδ
  have hs0 : 0 < Cscale := by linarith
  let B := C * Ccoeff ^ 4 * (1 + Real.log 16 + 6 / δ) * (Ca * Cscale ^ δ) *
    (16 : ℝ) ^ κ * (Csec + 1) * (16 * Cscale) ^ δ
  ...
  have ht := htau Cscale hscale x hx a ha
  ...
  have hApow : (wGramSecondaryNumeratorMax a K F j : ℝ) ^ δ ≤
      (16 * Cscale) ^ δ * x ^ (9 * δ) := by
    calc
      _ ≤ (16 * Cscale * x ^ (9 : ℕ)) ^ δ :=
        Real.rpow_le_rpow (by positivity) hA hδ.le
      _ = _ := by
        rw [Real.mul_rpow (by positivity) (by positivity), ← Real.rpow_natCast,
          ← Real.rpow_mul hx0.le]
        norm_num
  have hlarge : 1 ≤ (16 * Cscale) ^ δ * x ^ (9 * δ) := by
    have h1 := Real.one_le_rpow (show 1 ≤ 16 * Cscale by linarith) hδ.le
    have h2 := Real.one_le_rpow hx (show 0 ≤ 9 * δ by positivity)
    nlinarith
  have hroot : Real.sqrt (Csec * (wGramSecondaryNumeratorMax a K F j : ℝ) ^ δ) ≤
      (Csec + 1) * ((16 * Cscale) ^ δ * x ^ (9 * δ)) := by
    calc
      _ ≤ Real.sqrt (Csec * ((16 * Cscale) ^ δ * x ^ (9 * δ))) := by gcongr
      _ = Real.sqrt Csec * Real.sqrt ((16 * Cscale) ^ δ * x ^ (9 * δ)) :=
        Real.sqrt_mul hCsec _
      _ ≤ (Csec + 1) * ((16 * Cscale) ^ δ * x ^ (9 * δ)) := by
        apply mul_le_mul
  ...
  calc
    _ ≤ C * (Ccoeff ^ 4 * x ^ (4 * ρ)) *
        ((1 + Real.log 16 + 6 / δ) * x ^ δ) * (Ca * Cscale ^ δ * x ^ δ) *
        ((16 : ℝ) ^ κ * x ^ (4 * κ)) *
        ((Csec + 1) * ((16 * Cscale) ^ δ * x ^ (9 * δ))) := by gcongr
