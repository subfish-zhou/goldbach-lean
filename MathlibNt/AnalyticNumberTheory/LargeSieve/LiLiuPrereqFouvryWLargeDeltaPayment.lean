...
private theorem largeDelta_length_payment {x U ρ η : ℝ}
...
theorem eventually_wMaskedOriginal_zero_largeDelta_power_saving
    ...
    ∀ᶠ x : ℝ in atTop, ∀ M T : ℝ,
      ...
      |wMaskedOriginal M N Q β c a P| + |wMaskedZeroMode M N Q β c a P| ≤
        2 * M * T ^ 2 * x ^ (-ρ / 2) := by
  obtain ⟨C, hC, hb⟩ :=
    wMaskedOriginal_abs_le_largeDelta hk j (show 0 < ρ / 4 by linarith)
  let p := k ^ 2 - 1 + 2 * (k - 1)
  let s := 2 * j ^ 2 + 1
  ...
  have hMp := largeDelta_length_payment hx hM0 hρM hρη
  have hTp := largeDelta_length_payment hx hT0 hρT hρη
