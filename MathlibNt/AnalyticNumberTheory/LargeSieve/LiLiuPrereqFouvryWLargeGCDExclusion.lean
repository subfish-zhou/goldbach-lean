...
theorem wMaskedTruncated_abs_le_largeGCD
  ...
  obtain ⟨C, hC, hOriginal⟩ := wMaskedOriginal_abs_le_largeGCD hk j hε
...
theorem eventually_wMaskedTruncated_largeGCD_power_saving
  ...
  have hl' : 2 + η < η * (l : ℝ) := by
    ...
    nlinarith
  obtain ⟨C, D, hC, hD, hb⟩ :=
    wMaskedTruncated_abs_le_largeGCD hk hj l (show 0 < η / 8 by linarith)
  let p := k ^ 2 - 1
  let s := 2 * j ^ 2 + 1
  let v := 2 * (j - 1) + 2 * (k - 1)
...
theorem wMaskedTruncated_largeGCD_power_saving
  ...
  obtain ⟨x₀, hx₀⟩ := eventually_atTop.mp
    (eventually_wMaskedTruncated_largeGCD_power_saving hk hj hη)
...
theorem wMaskedTruncated_largeGCD_alpha_log_payment
  ...
  let u := i ^ 2 - 1
  let K : ℝ := 6 * 2 ^ u
  filter_upwards [eventually_wMaskedTruncated_largeGCD_power_saving hk hj hη,
...
theorem wMaskedTruncated_largeGCD_alpha_real_log_payment
    ...
    ∀ᶠ x : ℝ in atTop, ∀ M T : ℝ,
      ...
      (∑ m ∈ S, α m ^ 2) *
        |wMaskedTruncated M (wUniformCutoff M (x ^ η)) N Q β c a P| ≤
          x ^ 2 / (Real.log x) ^ A := by
  filter_upwards [wMaskedTruncated_largeGCD_alpha_log_payment hi hk hj ⌈A⌉₊ hη,
