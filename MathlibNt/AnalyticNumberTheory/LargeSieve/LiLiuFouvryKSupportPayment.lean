import MathlibNt.AnalyticNumberTheory.LargeSieve.LiLiuFouvryKSupport
...
private theorem kSupport_sqrt_rpow {x : ℝ} (hx : 0 < x) (η : ℝ) :
...
theorem eventually_wMaskedOriginal_zero_largeSupport_power_saving_kscale
    {k : ℕ} (hk : 1 ≤ k) (j : ℕ) {η Cscale : ℝ} (hη : 0 < η) (hCscale : 1 ≤ Cscale) :
    ∀ᶠ x : ℝ in atTop, ∀ M T : ℝ,
      ...
      |wMaskedOriginal M N Q β c a P| + |wMaskedZeroMode M N Q β c a P| ≤
        2 * M * T ^ 2 * x ^ (-η / 8) := by
  obtain ⟨C, hC, hb⟩ :=
    wMaskedOriginal_abs_le_largeSupport_kscale hk j
      (show 0 < η / 16 by linarith) hCscale
  let p := k ^ 2 - 1
  let s := 2 * j ^ 2 + 1
  ...
  have hZ :
    ...
    calc
      ...
      _ = M * |dyadicCutoffMass| *
        ...
        rw [abs_mul, abs_of_nonneg hM0, kSupport_sqrt_rpow hx0]
      _ ≤ M * |dyadicCutoffMass| *
