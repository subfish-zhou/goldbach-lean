...
import MathlibNt.AnalyticNumberTheory.LargeSieve.LiLiuPrereqFouvryDelayedConvolution
...
The original modulus weight is unchanged on the left. Its difference from
the trimmed convolution is paid at the original signed error, including the
equality progression. Only then is the accepted five-small-factor reduction
applied to the trimmed convolution and its second coefficient expanded.
-/
...
theorem signedError_sub_modulus (S N Q : Finset ℕ) (α β c d : ℕ → ℝ) (a : ℤ) :
    signedError S N Q α β c a - signedError S N Q α β d a =
      signedError S N Q α β (fun q => c q - d q) a := by
  simp only [signedError, sub_mul, sum_sub_distrib]

theorem signedError_sub_factorLowOmega (S N Q : Finset ℕ)
    (α β γ ζ : ℕ → ℝ) (a : ℤ) (ξ : ℝ) :
    signedError S N Q α β (factorConvolution γ ζ) a -
      signedError S N Q α β (factorConvolution γ (betaLowOmega ζ ξ)) a =
        signedError S N Q α β (factorConvolution γ (betaHighOmega ζ ξ)) a := by
  rw [signedError_sub_modulus, factorConvolution_sub_low]

/-- The difference is paid for original beta, not just its clean part.
The threshold precedes both chosen factors and every changing residue. -/
theorem factorLowOmega_signedError_difference_payment (i k u v A : ℕ)
    {ε : ℝ} (hε : 0 < ε) :
    ∀ᶠ x : ℝ in atTop, ∀ M T L : ℝ,
      ...
      |signedError S N Q α β (factorConvolution γ ζ) a -
        ...
        x / Real.log x ^ A := by
  filter_upwards [modulusHighOmega_signedError_dyadic_log_payment i k (u + v) A hε]
    with x hpay
...
def c2FiveSmallMask (x η : ℝ) (t : WOriginalTuple) : Prop :=
  ((((t.2.1.gcd t.2.2 : ℝ) ≤ x ^ η ∧
    ...
    ((wGCDTuple t).d₁ : ℝ) ≤ x ^ η) ∧
    ((wGCDTuple t).δ₁ : ℝ) ≤ x ^ η ∧ ((wGCDTuple t).δ₂ : ℝ) ≤ x ^ η

/-- A genuine reduction of the original error to the factored retained W.
The factor orders can differ and can be zero. The harmless coefficient is
eight after the additional square perturbation; no sign of W is assumed. -/
theorem signedError_sq_le_five_small_factored_c2
    ...
    ∀ᶠ x : ℝ in atTop, ∀ z : ι, ∀ M L R₀ S₀ : ℝ,
      ...
      signedError S (N z) Q α (β z) c a ^ 2 ≤
        8 * (∑ m ∈ S, α m ^ 2) *
          ...
          x ^ 2 / Real.log x ^ A := by
  filter_upwards [
    signedError_sq_le_lowOmega_five_small_truncated_c2
      (i := i) (j := u + v) (A + 2) hSW hT hN hβ hε hη,
    factorLowOmega_signedError_difference_payment i k u v (A + 2) hε,
...
theorem wellFactorable_signedError_sq_le_five_small_factored_c2
        ...
        ∃ γ ζ : ℕ → ℝ,
          ...
          signedError S (N z) Q α (β z) c a ^ 2 ≤
            8 * (∑ m ∈ S, α m ^ 2) *
              ...
              x ^ 2 / Real.log x ^ A := by
  filter_upwards [
    signedError_sq_le_five_small_factored_c2
      (i := i) (u := j) (v := j) A hSW hT hN hβ hε hη,
    eventually_ge_atTop (1 : ℝ)] with x hentry hx
  intro z M ν hM hscale hεν hν hpower S Q hS hQ α c hα hWF
  obtain ⟨hR, hS₀, hRS, _⟩ := c2_factor_levels hx hε.le hεν hν
  ...
  have hhigh : T z ≤ x ^ (1 / 10 : ℝ) := by
    rw [hpower]
    exact Real.rpow_le_rpow_of_exponent_le hx hν
  exact hentry z M _ _ _ hM hL hscale hlow hhigh hlevel
    S Q hS hQ α c γ ζ hα hγ hζ hγs hζs hc a ha hax

/-- Throughout the near-endpoint branch, the low-omega operation deletes
nothing from the second factor, not merely an asymptotically small error. -/
theorem factorLowOmega_eq_self_c2_near_endpoint {x ν ε ξ : ℝ}
    {ζ : ℕ → ℝ} (h : (1 - 10 * ν) / 9 ≤ ε / 2)
    (hζ : factorSupported (x ^ c2SExponent ν ε) ζ) (hξ : 0 ≤ ξ) :
    betaLowOmega ζ ξ = ζ := by
  apply factorLowOmega_eq_self_at_one _ hξ
  simpa only [c2SExponent_eq_zero_near_endpoint h, Real.rpow_zero] using hζ
