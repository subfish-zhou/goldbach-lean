...
import MathlibNt.AnalyticNumberTheory.LargeSieve.LiLiuFouvryKMainUniform
import MathlibNt.AnalyticNumberTheory.LargeSieve.LiLiuFouvryKSecondaryUniform
...
theorem direct_retained_cell_kscale (k m : ℕ) {Cscale ε η : ℝ}
    (hCscale : 1 ≤ Cscale)
    (hε : 0 < ε) (hη : 0 < η) (hη1 : η ≤ 1) (hηε : η < ε)
    (hbudget : 400*η ≤ ε/4) :
    ∃ C : ℝ, 0 < C ∧ ∀ (x M ν : ℝ),
      4 ≤ x → 1 ≤ M → ε ≤ ν → ν ≤ 1/10+ε/10 → x = 4*M*x^ν →
  ...
  obtain ⟨Bm,hBm,hmain⟩ := directPayKMain_uniform Cscale η η η η Cn Ct Cj Ca Cc Co
    hCscale hη.le hη hη.le hη.le hη1 hn.le ht.le hj.le ha.le ho.le
  obtain ⟨Bs,hBs,hsec⟩ := directJoinedKSecondary_uniform
    (κ := η) (δ := η) (ρ := η) (η := η) (Cnonzero := Cn)
    (Csecondary := Cs) (Ccoeff := Cc) (Couter := Co)
    hCscale hη.le hη hη.le hη.le hη1 hn.le hs.le ho.le
  ...
  obtain ⟨hR,hS,hRS,_⟩ := c2_extended_factor_levels hx1 hε hεν hν
  ...
  have hzmono := c2_extended_zero_monomial hx1 hε hεν hν
    (show 100*(η+η+η) ≤ ε/4 by linarith)
  have hmmono := c2_extended_main_monomial hx1 hε hεν hν
    (show 100*(η+η+η+η) ≤ ε/4 by linarith)
  have hsmono := c2_extended_secondary_monomial hx1 hε hεν hν
    (show 100*(η+η+η+η) ≤ ε/4 by linarith)
