...
theorem fourFactor_siegel_noZero_or_witness (η : ℝ) (hη : 0 < η) :
  ...
  have hδη : 48*δ ≤ η := by
    have hh : δ ≤ η/48 := (min_le_right _ _).trans (min_le_right _ _)
    linarith
  refine ⟨δ, hδ, hδ8, hδη, ?_⟩
  by_cases hw : ∃ ξ : PrimitiveQuadraticDatum, ∃ β : ℝ,
      β ∈ Ioo (1-δ) 1 ∧
      (let : NeZero ξ.modulus := ⟨ξ.modulus_ne⟩
       ξ.character.LFunction (β : ℂ) = 0)
  · exact Or.inl hw
  · apply Or.inr
    ...
    have hgt : δ/(4*(648*(q:ℝ))^(4*δ)) < (χ.LFunction 1).re := by
      by_contra hn
      obtain ⟨β, hβ, hz⟩ := hsmall δ hδ hδε q χ hprim hχ hquad (le_of_not_gt hn)
      exact hw ⟨⟨q, NeZero.ne q, χ, hprim, hquad, hχ⟩, β, hβ, hz⟩
    apply le_trans _ hgt.le
    calc
