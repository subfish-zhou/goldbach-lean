...
theorem eventually_log_rpow_le_rpow (b d : ℝ) (hd : 0 < d) :
  ...
  exact MathlibNt.Analysis.eventually_nat_log_rpow_le_rpow b d hd
...
theorem abs_li_sub_le_short (κ : ℝ) {x y : ℝ}
  ...
  have hid : liuLogarithmicIntegral κ y - liuLogarithmicIntegral κ x =
    ...
    exact intervalIntegral.integral_interval_sub_left
      (liuLogarithmicIntegrand_intervalIntegrable (hx.trans hxy))
      (liuLogarithmicIntegrand_intervalIntegrable hx)
  have hb : ‖∫ t in x..y, 1 / Real.log t‖ ≤ (1 / Real.log 2) * |y - x| := by
    apply intervalIntegral.norm_integral_le_of_norm_le_const
    intro t ht
    rw [Set.uIoc_of_le hxy] at ht
    have ht2 : 2 ≤ t := hx.trans ht.1.le
    rw [Real.norm_eq_abs, abs_of_nonneg (liuLogarithmicIntegrand_nonneg ht2)]
    exact one_div_le_one_div_of_le (Real.log_pos (by norm_num : (1 : ℝ) < 2))
      (Real.log_le_log (by norm_num) ht2)
  rw [hid]
  calc
    _ ≤ (1 / Real.log 2) * (y - x) := by
      simpa only [Real.norm_eq_abs, abs_of_nonneg (sub_nonneg.mpr hxy)] using hb
    _ ≤ 1 / Real.log 2 := by
      exact (mul_le_mul_of_nonneg_left (by linarith : y - x ≤ 1)
