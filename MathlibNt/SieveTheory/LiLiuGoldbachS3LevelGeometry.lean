...
private theorem S3Level_eventually_real_bound (B : ℝ) :
    ∀ᶠ N : ℕ in atTop, 4 ≤ N ∧
      2 * (N : ℝ) ^ (65 / 159 : ℝ) ≤
        (N : ℝ) ^ (1 / 2 : ℝ) / Real.log (N : ℝ) ^ B := by
  have hlog := LiuWeight.eventually_pan_log_rpow_le_rpow B (29 / 636) (by norm_num)
  have hconst : ∀ᶠ N : ℕ in atTop, (2 : ℝ) ≤ (N : ℝ) ^ (29 / 636 : ℝ) :=
    ((tendsto_rpow_atTop (by norm_num : (0 : ℝ) < 29 / 636)).comp
  ...
  have hgap : 2 * Real.log (N : ℝ) ^ B ≤ (N : ℝ) ^ (29 / 318 : ℝ) := by
    calc
      2 * Real.log (N : ℝ) ^ B ≤
          (N : ℝ) ^ (29 / 636 : ℝ) * (N : ℝ) ^ (29 / 636 : ℝ) := by
        gcongr
      _ = (N : ℝ) ^ (29 / 318 : ℝ) := by
  ...
  calc
    2 * (N : ℝ) ^ (65 / 159 : ℝ) * Real.log (N : ℝ) ^ B =
        (2 * Real.log (N : ℝ) ^ B) * (N : ℝ) ^ (65 / 159 : ℝ) := by ring
    _ ≤ (N : ℝ) ^ (29 / 318 : ℝ) * (N : ℝ) ^ (65 / 159 : ℝ) :=
...
theorem goldbachS3_level_eventually (B : ℝ) :
  ...
  filter_upwards [S3Level_eventually_real_bound B] with N hN
  refine ⟨hN.1, ?_⟩
  intro p hp hpu
  let z : ℝ := (N : ℝ) ^ (4 / 53 : ℝ)
  have hN0 : (0 : ℝ) < N := by exact_mod_cast (show 0 < N by omega)
  ...
  have hproduct : p * Nat.ceil z ≤ LiuWeight.panModulusCutoff N B := by
    ...
    calc
      ...
      _ ≤ (N : ℝ) ^ (1 / 3 : ℝ) * (2 * z) := by gcongr
      _ = 2 * (N : ℝ) ^ (65 / 159 : ℝ) := by
        dsimp [z]
        rw [← mul_assoc, mul_comm ((N : ℝ) ^ (1 / 3 : ℝ)) 2, mul_assoc,
          ← Real.rpow_add hN0]
        congr 2
        norm_num
