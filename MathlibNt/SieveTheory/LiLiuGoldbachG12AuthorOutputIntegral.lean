...
def goldbachG12AuthorIntegralConstant : ℝ :=
  (564383/1000000 : ℝ)*goldbachG12PrimeIntegral goldbachG11AuthorWeight
...
theorem original_total_integral (τ : ℝ) (hτ : 0 < τ) :
    ∀ ε : ℝ, 0 < ε → ε ≤ 2/15 → ∃ K : ℕ, 4 ≤ K ∧ ∀ N ≥ K, Even N →
      ...
      (goldbachG12AuthorIntegralConstant+τ)*
        (SingularSeries.liuSingularSeries N*(N : ℝ)/Real.log (N : ℝ)^2) := by
  let B := goldbachG12AuthorIntegralConstant
  ...
  obtain ⟨M,_,hmass⟩ := goldbachG12AuthorLowHigh_integral_budget t ht
  intro ε he heu
  ...
  have hi : Real.log (N : ℝ)/(N : ℝ)*(400*(Lm+8*Hm)) ≤ B+t := hmass N (by omega) ε
