...
theorem goldbachG12Linked_outputDivisors_empty {N m d : ℕ} {ε : ℝ}
  ...
  rw [Nat.sub_add_cancel (goldbachG12LinkedPrimeWindow_product_le hm hr)] at hgN
...
noncomputable def goldbachG12LinkedDivisorResidual (N : ℕ) (ε : ℝ) (d : ℕ) : ℝ :=
...
theorem goldbachG12LinkedDivisorResidual_eq {N : ℕ} (hN : 2 ≤ N)
    (ε : ℝ) (d : ℕ) (hNd : N.Coprime d) :
    goldbachG12LinkedDivisorResidual N ε d = goldbachG12LinkedWindowResidual N ε d N := by
  classical
  have hc : (∑ m ∈ goldbachG12ActiveProductSupport N,
    ...
    · rw [if_neg hmd, goldbachG12Linked_outputDivisors_empty hm hNd hmd]
      simp
  unfold goldbachG12LinkedDivisorResidual goldbachG12LinkedWindowResidual
  rw [hc, Finset.sum_div, ← sum_sub_distrib]
  apply sum_congr rfl
  intro m hm
  obtain ⟨hm, hmd⟩ := mem_filter.mp hm
  have hcount := goldbachG12LinkedAPWindow_card_eq_inverse (ε := ε) d N hN hm hmd
  rw [goldbachG12LinkedAPWindow_eq_output_dvd d hm] at hcount
  rw [hcount]
  ring
...
theorem goldbachG12LinkedDivisorResidual_log_saving (A : ℝ) (hA : 0 < A) :
    ∃ B C : ℝ, 0 < B ∧ 0 < C ∧ ∃ N₀ : ℕ, 4 ≤ N₀ ∧
      ∀ N ≥ N₀, ∀ (ε : ℝ) (Q : ℕ),
      (Q : ℝ) ≤ Real.sqrt N / Real.log (N : ℝ) ^ B →
      (∑ d ∈ goldbachG11LinkedModuli N Q,
        |goldbachG12LinkedDivisorResidual N ε d|) ≤ C*N / Real.log (N : ℝ)^A := by
  obtain ⟨B, C, hB, hC, N₀, hN₀, hbound⟩ := goldbachG12LinkedWindowResidual_squarefree A hA
  refine ⟨B, C, hB, hC, N₀, hN₀, ?_⟩
  intro N hN ε Q hQ
  have hN2 : 2 ≤ N := by omega
  have heq : (∑ d ∈ goldbachG11LinkedModuli N Q,
      |goldbachG12LinkedDivisorResidual N ε d|) =
      ∑ d ∈ goldbachG11LinkedModuli N Q, |goldbachG12LinkedWindowResidual N ε d N| := by
    apply sum_congr rfl
    intro d hd
    rw [goldbachG12LinkedDivisorResidual_eq hN2 ε d (mem_filter.mp hd).2.2]
  rw [heq]
