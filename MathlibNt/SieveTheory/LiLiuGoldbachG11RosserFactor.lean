import MathlibNt.SieveTheory.LiLiuGoldbachG11SmallOutput
...
theorem exists_goldbachG11LinkedBoundingSieve_dimensionOne :
    ∃ K : ℝ, 1 < K ∧ ∀ (N : ℕ) (hEven : Even N) (ε Z X : ℝ),
      HasDimensionOneLocalProductBound (goldbachG11LinkedBoundingSieve N hEven ε Z X) K := by
...
theorem goldbachG11Linked_upperRosserCertificate {N : ℕ} {Z Δ s : ℝ}
    (hZ : 2 ≤ Z) (hΔ : 0 < Δ) (hs : s = Real.log Δ / Real.log Z) (hslo : 3/2 ≤ s) :
    LinearSieve.IsUpperRosserCertificate (goldbachB10ProdPrimes N Z) (Nat.floor Δ+1) := by
  have hD : 1 < Nat.floor Δ+1 := by
    have htwo := lt_floor_add_one_of_le_of_log_ratio (p := 2) hZ hΔ hZ hs hslo
    omega
  have hcut : ∀ p ∈ (goldbachB10ProdPrimes N Z).primeFactors, p < Nat.floor Δ+1 := by
    ...
    exact lt_floor_add_one_of_le_of_log_ratio hZ hΔ
      (prime_dvd_goldbachB10ProdPrimes_lt hprime hpdvd).le hs hslo
  exact LinearSieve.upperRosserWeight_certificate (goldbachB10ProdPrimes_squarefree N Z)
    (goldbachB10ProdPrimes_ne_zero N Z) hD hcut

/-- Actual weighted G11 sieve, consuming the already constructed all-depth source.
The precise finite remainder is retained for the common-distribution consumer. -/
theorem goldbachG11LinkedSiftedMass_le_rosserFactor (ρ : ℝ) (hρ : 0 < ρ) :
    ∃ z₀ : ℝ, ∀ (N : ℕ) (hEven : Even N) (ε Z Δ s X : ℝ),
      ...
      goldbachG11LinkedSiftedMass N ε Z ≤
  ...
  obtain ⟨K, hK, hlocal⟩ := exists_goldbachG11LinkedBoundingSieve_dimensionOne
  ...
  let P := goldbachB10ProdPrimes N Z
  let D := Nat.floor Δ+1
  have hcert := goldbachG11Linked_upperRosserCertificate (N := N) hZ hΔ hs hslo
  ...
  have hf' : goldbachG11LinkedSiftedMass N ε Z ≤
      X*S.mainSum (LinearSieve.upperRosserWeight P D) +
      LinearSieve.upperErrSum S D (LinearSieve.upperRosserWeight P D) := by
    change (goldbachG11LinkedBoundingSieve N hEven ε Z X).siftedSum ≤ _ at hf
    rw [goldbachG11LinkedBoundingSieve_siftedSum] at hf
    exact hf
