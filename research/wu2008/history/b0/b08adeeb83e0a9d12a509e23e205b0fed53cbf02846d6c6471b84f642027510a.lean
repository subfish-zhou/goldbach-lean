...
theorem J_source_identity {f : ℝ → ℝ} (hf : IntervalIntegrable f volume 1 3)
  ...
  have hb : IntervalIntegrable (fun t =>
      ...
      volume 1 3 := by
    have hind := ((intervalIntegrable_iff_integrableOn_Icc_of_le
      (by norm_num : (1 : ℝ) ≤ 3)).mp (profile_div_integrable hf)).indicator
        (s := Icc (S - 2) 3) measurableSet_Icc
    have hind' := (intervalIntegrable_iff_integrableOn_Icc_of_le
      (by norm_num : (1 : ℝ) ≤ 3)).mpr hind
    convert (sigma_weight_integrable hf).add hind' using 1
    ext t
    by_cases ht : t ∈ Icc (S - 2) 3 <;> simp [ht] <;> ring
  have hpoint (t : ℝ) :
      f t * JKernel s S t =
      log ((S - 1) / (s - 1)) *
        (f t * (sigma0 t + (Icc (S - 2) 3).indicator (fun _ => (1 : ℝ)) t) / t) +
    ...
    dsimp [JKernel]
    ring
  simp_rw [he, he', (J_source_logs (t := ·) hs hS).2] at hi
  constructor
  ...
  · simp_rw [hpoint]
    ...
    unfold source63RHS
    simp_rw [he, he', (J_source_logs (t := ·) hs hS).2]
