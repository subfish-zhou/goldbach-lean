...
theorem gamma78Gain_region_measurable (tri : Bool) : MeasurableSet (gamma78GainRegion tri) := by
  have hs : Continuous (gamma78GainStart tri) := by
    cases tri <;> first | exact continuous_const | exact continuous_id
...
theorem gamma78Gain_ratio_ne_ae (r : ℝ) :
    ∀ᵐ v : ℝ × ℝ, gamma78GainV v.1 v.2 ≠ r := by
  filter_upwards [gamma5Gain_vertical_ne_ae 0,
    gamma5Gain_affine_ne_ae (r+1) 1 1 (by norm_num)] with v ht hv
  intro he
  exact hv (by simpa only [one_mul] using (gamma78Gain_ratio_level_line ht).mp he)
...
theorem gamma78Gain_H_pullback_ae {δ : ℝ} (hδ : 0 < δ) (hδhi : δ < 1/2) :
  ...
  letI : Countable {s : ℝ // ¬ ContinuousAt g s} := hcount.to_subtype
  have havoid : ∀ᵐ v : ℝ × ℝ, ∀ r : {s : ℝ // ¬ ContinuousAt g s},
      gamma78GainV v.1 v.2 ≠ r.val :=
    ae_all_iff.mpr (fun r => gamma78Gain_ratio_ne_ae r.val)
  have hae : ∀ᵐ v : ℝ × ℝ, ContinuousAt g (gamma78GainV v.1 v.2) := by
    filter_upwards [havoid] with v hv
    by_contra hn
    exact hv ⟨gamma78GainV v.1 v.2,hn⟩ rfl
  filter_upwards [hae] with v hc
  have hh := hc.neg
  have he : -g = gamma5GainH δ := by funext s; exact neg_neg _
  rwa [he] at hh
...
theorem gamma78Gain_C_inner_integrable (tri : Bool) {t : ℝ} (ht : t ∈ Icc gamma5MassA gamma6BaseB) :
  ...
  apply ((gamma5Gain_smooth_continuous.comp
    (continuous_const.prodMk continuous_id)).intervalIntegrable
      (gamma78GainStart tri t) (gamma78GainUpper tri)).congr
  intro u hu
...
theorem gamma78Gain_C_outer_integrable (tri : Bool) :
  ...
  have hc : Continuous (fun t : ℝ =>
      ∫ u in gamma78GainStart tri t..gamma78GainUpper tri, gamma5GainSmooth (t,u)) :=
    gamma5Gain_moving_integral (f := fun t u => gamma5GainSmooth (t,u))
      gamma5Gain_smooth_continuous hs continuous_const
  apply (hc.intervalIntegrable gamma5MassA gamma6BaseB).congr
  intro t ht
