...
theorem gamma5Gain_mass_summand_nonneg {i k N : ℕ} {δ Δ : ℝ} {V : Fin i → ℝ}
  ...
  have hs := wuSingularSeries_pos _ (Nat.mul_pos hg.product_pos (by omega))
...
theorem gamma5Gain_packing_contains_inner (k : ℕ) {δ τ : ℝ}
  ...
  have hstepτ : gamma5GainStep (gamma5GainScale N δ V) Δ < τ / 2 :=
    (le_mul_of_one_le_left hstep.le (by positivity : (1 : ℝ) ≤ (k : ℝ) + 1)).trans_lt hsmall
  ...
  have hqR : (x.2.2 : ℝ) ≤ (N : ℝ) ^ (1 / 2 - δ) / x.1 :=
    hqu.le.trans (by simpa only [rpow_one] using
