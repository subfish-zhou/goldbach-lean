...
/-!
# The actual Gamma5 classical upper count

This is an unconditional producer for every actual finite label mask.
The main mass contains the true logarithmic integral, the singular series
at `dpqN`, the totient of `dpq`, and the original local logarithm.
It is the classical component only, not the full admissible-H Gamma5 bound.
-/
...
theorem gamma5Classical_density_eventually (k : ℕ) {δ η : ℝ}
  ...
  have hζ := (gamma5Classical_exponents_pos k hδ hδhi).2
  obtain ⟨ZD, hden⟩ := ordinaryRosser_upper_density_canonical_bounded_local hρ
  obtain ⟨ZE, hEuler⟩ := eventually_atTop.mp
    (eventually_localSieveProduct_relative (2 / gamma5ClassicalZeta k δ) η
      (by positivity) hη)
  have hlarge : ∀ᶠ N : ℕ in atTop,
      max 2 (max ZD ZE) ≤ (N : ℝ) ^ gamma5ClassicalZeta k δ :=
  ...
  have hr3 : r ≤ 3 := hg.ratio_upper.trans (by norm_num [gamma5ClassicalS])
  ...
  have hnorm := hEuler z hzE (gamma5ClassicalProduct x * N) hMN
    (he.mul_left _) (gamma5Classical_polynomial_envelope hζ hg)
...
theorem gamma5Classical_label_finite_upper {i k N : ℕ} {δ Δ : ℝ}
  ...
  have hsame := gamma5Classical_source_count_eq hp hq hzp
...
theorem gamma5Classical_mask_upper (k : ℕ) (_hk : 1 ≤ k) {δ η ε : ℝ}
  ...
  obtain ⟨TD, hden⟩ := gamma5Classical_density_eventually k hδ hδhalf hη
  obtain ⟨TR, hrem⟩ := gamma5Classical_remainder_relative k hδ hδhalf hε
      ...
      (sourceSieveCount N (gamma5ClassicalProduct x) (x.1 * N)
        ...
        (logarithmicIntegral N / (Nat.totient (gamma5ClassicalProduct x) : ℝ)) *
          ...
          ordinaryRosserRemainder true N (gamma5ClassicalProduct x)
            ...
            (gamma5ClassicalCutoff N δ x) := by
    intro x hx
    apply (gamma5Classical_label_finite_upper hN hδ hδhalf hbox (hX hx)).trans
    exact add_le_add_right (mul_le_mul_of_nonneg_left
      (hden N hND he i Δ V hbox x (hX hx)) (div_nonneg hli (Nat.cast_nonneg _))) _
  have hfinite :
      gamma5ClassicalCount N δ (convolutionWuWindows N Δ V) X ≤
        (1 + η) ^ 2 * gamma5ClassicalMainMass N δ (convolutionWuWindows N Δ V) X +
    ...
    convert h using 1 <;> dsimp only [gamma5ClassicalLevel] <;> ring
  exact hfinite.trans (add_le_add_left
    ((le_abs_self _).trans (hrem N hNR i Δ V hbox X hX (gamma5ClassicalCutoff N δ))) _)

/-- Explicit full-mask consumer of the unconditional count producer. -/
theorem gamma5Classical_full_upper (k : ℕ) (hk : 1 ≤ k) {δ η ε : ℝ}
    (hδ : 0 < δ) (hδhi : δ ≤ 1 / 10) (hη : 0 < η) (hε : 0 < ε) :
    ∃ T : ℕ, 4 ≤ T ∧ ∀ N : ℕ, T ≤ N → Even N →
      ∀ i : ℕ, ∀ Δ : ℝ, ∀ V : Fin i → ℝ, wuSourceBox k δ N i Δ V →
        gamma5ClassicalCount N δ (convolutionWuWindows N Δ V)
          ...
          (1 + η) ^ 2 * gamma5ClassicalMainMass N δ (convolutionWuWindows N Δ V)
            (gamma5ClassicalLabels N δ (convolutionWuWindows N Δ V)) +
          ε * boxTheta N ((N : ℝ) ^ (1 / 2 - δ)) (convolutionWuWindows N Δ V) := by
  obtain ⟨T, hT, h⟩ := gamma5Classical_mask_upper k hk hδ hδhi hη hε
