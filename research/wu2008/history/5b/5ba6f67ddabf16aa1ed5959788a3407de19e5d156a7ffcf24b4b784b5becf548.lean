...
theorem first_source63_endpoints {s S : ℝ} (hs : 0 < s) (hS : 0 < S) :
    (1 - 1 / s) * S - 1 = S - S / s - 1 ∧
    (1 - 1 / S) * S - 1 = S - 2 := by
  constructor
  · ring
...
theorem firstFeedback_eq_original_integral (i : Fin 5) (z : Fin 9 → ℝ) :
  ...
  · have he : i = (4 : Fin 5) := Fin.ext (by omega)
    subst i
    convert Wu04Source.firstFeedback_eq_paper_integral z using 1 <;>
      norm_num [firstNode, firstS]
