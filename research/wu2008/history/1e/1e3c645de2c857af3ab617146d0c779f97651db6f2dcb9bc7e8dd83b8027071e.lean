import MathlibNt.Wu2008DoubleSieve.FourthRowTripleNoGateMain
import MathlibNt.Wu2008DoubleSieve.Omega3XIntegralNormalization

/-! # Sharp normalization on the unchanged original boxTheta -/
...
theorem fourthRowTripleNoGate_integral_scaled {i k N : ℕ} {δ Δ K τ : ℝ}
    {V : Fin i → ℝ} (hN : 4 ≤ N) (hδ : 0 < δ) (hδhi : δ < 1 / 2)
    (hb : wuSourceBox k δ N i Δ V) (hK : 0 ≤ K)
    (hli : (N : ℝ) / log N ≤ (1 + τ) * logarithmicIntegral N) (eleven : Bool) :
    let W := convolutionWuWindows N Δ V
    fourthRowTripleNoGateIntegralMain N δ W eleven * (K * wuSingularSeries N / log N) ≤
      ((1 + τ) * K / 4 * fourthRowTripleNoGateEnvelope eleven) *
        boxTheta N ((N : ℝ) ^ (1 / 2 - δ)) W := by
  let W := convolutionWuWindows N Δ V
  let Q := (N : ℝ) ^ (1 / 2 - δ)
  let E := fourthRowTripleNoGateEnvelope eleven
  ...
  have hE : 0 ≤ E := (fourthRowTripleNoGate_envelope_bounds eleven).1
  ...
  have he : fourthRowTripleNoGateIntegralMain N δ W eleven ≤ (N : ℝ) * E * A := by
    unfold fourthRowTripleNoGateIntegralMain
    rw [mul_assoc]
    ...
    simpa only [mul_comm] using mul_le_mul_of_nonneg_left
      (fourthRowTripleNoGate_le_envelope eleven hφ)
...
theorem fourthRowTripleNoGate_X_scaled (k : ℕ) {δ K τ ε : ℝ}
    (hδ : 0 < δ) (hδhi : δ < 1 / 2) (hK : 0 ≤ K) (hτ : 0 < τ) (hε : 0 < ε) :
    ∃ T : ℕ, 4 ≤ T ∧ ∀ N : ℕ, T ≤ N →
      ∀ (i : ℕ) (Δ : ℝ) (V : Fin i → ℝ), wuSourceBox k δ N i Δ V →
      ∀ eleven : Bool,
      let W := convolutionWuWindows N Δ V
      fourthRowTripleNoGateX N δ W eleven * (K * wuSingularSeries N / log N) ≤
        (((1 + τ) * K / 4 * fourthRowTripleNoGateEnvelope eleven) + ε * K / 2) *
          boxTheta N ((N : ℝ) ^ (1 / 2 - δ)) W := by
  obtain ⟨TM, hTM4, hTM⟩ := fourthRowTripleNoGate_X_main k hδ hδhi hε
  ...
  have hm := hTM N ((le_max_left _ _).trans hN) i Δ V hb eleven
  have hn := fourthRowTripleNoGate_integral_scaled hN4 hδ hδhi hb hK
    (hTL N ((le_max_right _ _).trans hN)) eleven
