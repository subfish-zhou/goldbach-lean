...
theorem gamma5Gain_affine_ne_ae (a b c : ℝ) (hb : b ≠ 0) :
  ...
  exact ae_iff.mpr ((Set.countable_singleton c).preimage hinj).measure_zero
...
theorem gamma5Gain_vertical_ne_ae (a : ℝ) : ∀ᵐ v : ℝ × ℝ, v.1 ≠ a := by
  ...
  rw [Measure.ae_prod_iff_ae_ae hm]
  filter_upwards [show ∀ᵐ t : ℝ, t ≠ a from ae_iff.mpr (Set.countable_singleton a).measure_zero]
    with t ht
