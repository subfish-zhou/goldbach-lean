...
theorem D1_derivative {x : ℝ} (hx : x ≠ 0) (hl : lam-x ≠ 0) :
  ...
  convert hd using 1 <;> first | rfl | (dsimp; unfold D2; ring)
...
theorem D_bounds {x : ℝ} (hx : x ∈ Icc alpha beta) :
  ...
  have hc1 : |1+cross x| ≤ (5/2 : ℝ) := by
    have h := abs_add (1 : ℝ) (cross x)
    norm_num at h
    linarith only [h,hc]
  have hc2 : |3+2*cross x| ≤ (6 : ℝ) := by
    have h := abs_add (3 : ℝ) (2*cross x)
    rw [abs_mul] at h
    norm_num at h
    linarith only [h,hc]
  constructor
  ...
  · unfold D2
    calc
      _ ≤ |-((lam-x)⁻¹)^2*x⁻¹+2*(lam-x)⁻¹*(x⁻¹)^2|+
          |(3+2*cross x)*(x⁻¹)^3| := abs_add _ _
      _ ≤ |-((lam-x)⁻¹)^2*x⁻¹|+|2*(lam-x)⁻¹*(x⁻¹)^2|+
          |(3+2*cross x)*(x⁻¹)^3| := add_le_add_right (abs_add _ _) _
      _ = |(lam-x)⁻¹|^2*|x⁻¹|+2*|(lam-x)⁻¹|*|x⁻¹|^2+
          |3+2*cross x|*|x⁻¹|^3 := by
        simp only [abs_mul,abs_neg,abs_pow,abs_ofNat]
  ...
  have hq2 : |w x/x^2-2*W x/x^3| ≤ 43120 := by
    calc
      ...
      _ = |w x|*|x⁻¹|^2+2*|W x|*|x⁻¹|^3 := by
        simp only [div_eq_mul_inv,inv_pow,abs_mul,abs_pow,abs_ofNat]
      _ ≤ 108*(14 : ℝ)^2+2*4*14^3 := by gcongr
      _ = _ := by norm_num
  unfold F2
  calc
    _ ≤ |(w x/x^2-2*W x/x^3)*D x+2*(W x/x^2)*D1 x|+
        |Q x*D2 x| := abs_add _ _
    _ ≤ |(w x/x^2-2*W x/x^3)*D x|+|2*(W x/x^2)*D1 x|+
        |Q x*D2 x| := add_le_add_right (abs_add _ _) _
    _ = |w x/x^2-2*W x/x^3|*|D x|+2*|W x/x^2|*|D1 x|+
        |Q x|*|D2 x| := by simp only [abs_mul,abs_ofNat]
