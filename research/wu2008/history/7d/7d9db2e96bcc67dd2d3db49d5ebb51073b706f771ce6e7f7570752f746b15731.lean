...
theorem rectangle_quotient_sum (N : ℕ) {z w u V : ℝ}
  ...
  have h := sum_sdiff (f := fourQuotientTerm N) (moving_subset_rectangle N hu hV)
...
theorem rectangle_modulus_cancellation (N : ℕ) {z w u V : ℝ}
  ...
  have hq := rectangle_quotient_sum N hu hV
...
theorem signed_aggregate_transport (N : ℕ) {z w u v V : ℝ}
  ...
  have hc := congrArg (fun x : ℤ => (x : ℝ)) (rectangle_modulus_cancellation N hu hV)
  push_cast at hm hc
