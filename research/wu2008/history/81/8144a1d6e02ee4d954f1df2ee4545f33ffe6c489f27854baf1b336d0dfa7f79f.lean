...
theorem high_log_lower {s : ℝ} (hs : s ≤ FifthClassicalShape.q) :
  ...
  have he : (1 : ℝ)+FifthClassicalShape.q-s = 1/(2*a)-1-s := by
    ...
    field_simp [ha]
    ring
  simpa only [sub_add_cancel, div_one, show (1 : ℝ)+FifthClassicalShape.q-s-1 =
    FifthClassicalShape.q-s by ring,
    show (1 : ℝ)+FifthClassicalShape.q-s+1 = 2+FifthClassicalShape.q-s by ring, he] using h
...
theorem lower_envelope {l s : ℝ} (hl : s0 ≤ l) (hls : l ≤ s)
  ...
  have hn : 0 ≤ 2*a*(s-s0) := by positivity
