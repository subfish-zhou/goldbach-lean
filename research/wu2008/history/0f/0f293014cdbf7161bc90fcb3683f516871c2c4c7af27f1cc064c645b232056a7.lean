  ...
  for (n, c) in env.constants.toList do
    ...
    if s.startsWith "Wu2008DoubleSieve" && !s.contains "_proof" &&
      ...
      if cs.contains `Wu2008DoubleSieve.wuOmega1 ||
          ...
          cs.contains `Wu2008DoubleSieve.secondFunctionalMotherGamma ||
          cs.contains `Wu2008DoubleSieve.secondFunctionalMotherRHS ||
          (s.startsWith "Wu2008DoubleSieve.secondFunctionalMother" &&
            (s.contains "finite" || s.contains "le_")) then
        logInfo m!"{n} : {c.type}"
