...
elab "#w11_credit_parent_cone" : command => do
  let env := (← getEnv).setExporting false
  for root in #[`WuTarget.W11Credit.payment_le_credit,
      `WuTarget.W11CreditAccepted.paidCoefficient, `WuTarget.W11CreditAccepted.refined_target] do
    let mut todo := #[root]
