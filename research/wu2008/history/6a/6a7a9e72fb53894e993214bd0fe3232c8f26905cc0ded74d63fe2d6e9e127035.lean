...
theorem cell_enclosure {a h : ℝ} {c d : Fin 7 → ℝ} (ha : 2 ≤ a)
  ...
  have h0 : 0 ≤ f 0 := by
    dsimp [f, poly]
    simp only [zero_pow (by omega : (1 : ℕ) ≠ 0), zero_pow (by omega : (2 : ℕ) ≠ 0),
...
theorem certified_prefix (C : ℕ → Fin 7 → ℝ)
  ...
  | h i ih =>
    by_cases hi20 : i < 20
    · apply initial_enclosure (by dsimp [left]; positivity) hx hxh _
        (base i hi20).1 (base i hi20).2
      have hi' : (i : ℝ) ≤ 19 := by exact_mod_cast (show i ≤ 19 by omega)
      dsimp [left]
      linarith
    · have hi20' : 20 ≤ i := by omega
      apply cell_enclosure (by dsimp [left]; exact_mod_cast
        (show (2 : ℚ) ≤ 1 + (i : ℚ) / 20 by
          have hh : (20 : ℚ) ≤ i := by exact_mod_cast hi20'
          linarith)) _ _ (step i hi20' hi) hx hxh
      · by_cases hieq : i = 20
        · subst i
