...
theorem lowerLog_nonneg {x : ℝ} (hx : 1 ≤ x) :
  ...
  simpa only [Wu04WholeCollection.low, if_pos hx, Wu04FactorEnvelopes.lower] using h
...
theorem jCoefficient_nonneg {s S : ℝ} (hs : 2 ≤ s) (hS : 3 ≤ S)
  ...
  rw [← CoupledJLogRecovery.kernel_integral
    (a := left (jStart s S - 1) (S - 2) k)
    (b := right (jStart s S - 1) (S - 2) k) hA.1 hA.2
...
theorem terminal_coefficient_zero (k : Fin 9) : jCoefficient 3 3 k = 0 := by
  norm_num [jCoefficient, jStart, left, right, clip]
