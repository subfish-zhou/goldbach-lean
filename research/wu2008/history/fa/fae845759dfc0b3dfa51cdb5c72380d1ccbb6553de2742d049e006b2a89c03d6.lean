import R2GammaHighPairGeometry
...
theorem main_rect_identity {δ : ℝ} (hd : 0 < δ) (hh : δ ≤ 1/100) :
  ...
  have hp4 := (hlarge N (by omega)).trans (prime_lower hg.2.2.2 hA hZ pq.1 hp).2
  have hq4 := (hlarge N (by omega)).trans (prime_lower hg.2.2.2 hC hZ pq.2 hq).2
...
theorem rectangle_mass {δ ε : ℝ}
  ...
  have hli : 0 ≤ 4*logarithmicIntegral N := mul_nonneg (by norm_num)
    ((by positivity : (0 : ℝ) ≤ N/(2*log N)).trans (HighTheta.box_trueLi_lower hN4))
...
theorem term_mass {δ ε : ℝ}
  ...
  have he : 1/(Wu04RemainingCore.row j).kappa3 ≤ (10/23 : ℝ) := by
    fin_cases j <;> norm_num [Wu04RemainingCore.row, ActualNineFeedback.coupledRow,
  ...
  cases i with
  | gammaFive => exact hm N hN j _ _ _ _ ha (hab.trans hbc.le) (hce.le.trans he)
      ha (hab.trans hbc.le) (hce.le.trans he)
  | gammaSix => exact hm N hN j _ _ _ _ ha hab (hbc.le.trans (hce.le.trans he))
      (ha.trans (hab.trans hbc.le)) hce.le he
  | gammaSeven => exact hm N hN j _ _ _ _ ha hab (hbc.le.trans (hce.le.trans he))
      ha hab (hbc.le.trans (hce.le.trans he))
  | gammaEight => exact hm N hN j _ _ _ _ ha hab (hbc.le.trans (hce.le.trans he))
