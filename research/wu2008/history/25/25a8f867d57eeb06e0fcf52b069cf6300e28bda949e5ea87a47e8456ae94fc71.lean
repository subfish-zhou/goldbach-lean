...
theorem first_profile (w : Fin 21 → ℝ) {s : ℝ}
  ...
  change Wu08Staircase.eval
    ((20/10,21/10,w 0) :: (List.finRange 21).tail.map (DirectFiniteF6.row w)) s = w 0
  exact Wu08Staircase.eval_hit _ _ _ _ _ (by norm_num; exact hs)
...
theorem first_cell_surplus {x : Fin 9 → ℝ} (hx : ∀ i, 0 ≤ x i)
  ...
  have hn : hContinuous x (21/10) = matrixApply transferMatrix x 0 := by
    convert hContinuous_node x 0 using 1
    norm_num [rNode]
...
theorem rectangle_kernel_surplus {x : Fin 9 → ℝ} (hx : ∀ i, 0 ≤ x i)
  ...
  have hm := mul_le_mul_of_nonneg_left hupper
    (show 0 ≤ 125*x 0/11 from
      div_nonneg (mul_nonneg (by norm_num) (hx 0)) (by norm_num))
  nlinarith only [h,hm]
