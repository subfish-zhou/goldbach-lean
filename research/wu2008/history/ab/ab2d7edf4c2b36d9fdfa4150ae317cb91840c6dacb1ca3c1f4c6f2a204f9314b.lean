...
theorem gamma78Gain_ratio_drift {i k N p q : ℕ} {δ Δ : ℝ} {V : Fin i → ℝ}
  ...
  have hratio : log ((N : ℝ)^(1/2-δ)/d)/log (gamma5GainScale N δ V)-1 ≤
      (i : ℝ)*log Δ/log (gamma5GainScale N δ V) := by
    have hh := div_le_div_of_nonneg_right hhi hlog.le
    rw [add_div, div_self hlog.ne'] at hh
    linarith
  have hratio0 : 0 ≤ log ((N : ℝ)^(1/2-δ)/d)/log (gamma5GainScale N δ V)-1 := by
    have hh : 1 ≤ log ((N : ℝ)^(1/2-δ)/d)/log (gamma5GainScale N δ V) :=
      (le_div_iff₀ hlog).mpr (by simpa only [one_mul] using hlo)
    linarith
  ...
  exact add_le_add_right
    ((div_le_div_of_nonneg_left hratio0 ha ht).trans (div_le_div_of_nonneg_right hratio ha.le)) _
...
theorem gamma78Gain_rectangle_admission (k : ℕ) {δ : ℝ} {tri : Bool}
  ...
  · cases tri
    ...
    · trivial
  · have hdr := gamma78Gain_ratio_drift (q := x.2.2) hN hδ hδhi hb had.1 had.2.1 hd hp.one_lt (r.box.lower.le.trans hpc.1)
