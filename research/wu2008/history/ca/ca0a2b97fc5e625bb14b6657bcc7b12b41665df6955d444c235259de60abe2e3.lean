...
theorem gamma78Gain_H_pullback_ae {δ : ℝ} (hδ : 0 < δ) (hδhi : δ < 1/2) :
  ...
  have hv : Measurable (fun v : ℝ × ℝ => gamma78GainV v.1 v.2) := by
    unfold gamma78GainV
    fun_prop
  have hm : MeasurableSet {v : ℝ × ℝ | ContinuousAt g (gamma78GainV v.1 v.2)} :=
    (measurableSet_of_continuousAt g).preimage hv
  have hae : ∀ᵐ v : ℝ × ℝ, ContinuousAt g (gamma78GainV v.1 v.2) := by
    ...
    filter_upwards [show ∀ᵐ t : ℝ, t ≠ 0 from ae_iff.mpr ((Set.countable_singleton 0).measure_zero volume)]
      with t ht
