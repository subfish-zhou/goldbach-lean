...
theorem payment_le_actual :
    payment ≤ 3 * firstMain - thirdMain - fourthMain :=
  (add_le_add_right W11Credit.payment_le_credit jointGain).trans correlated_gain_le_actual
...
theorem payment_le_selected : payment ≤ selectedCredit :=
  (add_le_add_right W11Credit.payment_le_credit jointGain).trans (le_max_right _ _)
