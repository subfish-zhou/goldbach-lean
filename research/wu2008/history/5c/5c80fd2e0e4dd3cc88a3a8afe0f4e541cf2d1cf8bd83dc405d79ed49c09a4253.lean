...
theorem profileJ_split (z : Fin 9 → ℝ) {s S : ℝ}
  ...
  unfold jRemainder jCoefficient
  ring
...
theorem eProfile_sigma_lower {z : Fin 9 → ℝ} (hz : ∀ k, 0 ≤ z k)
  ...
  rw [eProfile_split]
  exact add_le_add
    (mul_le_mul_of_nonneg_left (sigmaLower_le_aProfile hz) (eCoefficient_nonneg hS hS5)) le_rfl
...
theorem profileJ_sigma_lower {z : Fin 9 → ℝ} (hz : ∀ k, 0 ≤ z k)
  ...
  exact add_le_add
    (mul_le_mul_of_nonneg_left (sigmaLower_le_aProfile hz) (jCoefficient_nonneg hs hsS)) le_rfl
...
theorem firstFeedback_sigma_lower {z : Fin 9 → ℝ} (hz : ∀ k, 0 ≤ z k)
  ...
  exact add_le_add (mul_le_mul_of_nonneg_left (sigmaLower_le_aProfile hz)
    (firstCoefficient_nonneg hs hS hS5 hsS)) le_rfl
...
theorem coupledFeedback_sigma_lower {p : SecondFunctionalParameters} (hp : CoupledGeometry p)
  ...
  exact add_le_add (mul_le_mul_of_nonneg_left (sigmaLower_le_aProfile hz)
    (coupledCoefficient_nonneg hp)) le_rfl
