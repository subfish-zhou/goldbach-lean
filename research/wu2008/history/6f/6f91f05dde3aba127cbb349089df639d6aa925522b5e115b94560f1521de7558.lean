...
theorem thirteen_separate_sum (p : SecondFunctionalParameters) :
  ...
  have ht (j : Fin 6) : rawTriple p j = rawIntegral 1 (tripleDomain p j) := rfl
  have hf (j : Fin 4) : rawFour p j = rawIntegral 2 (fourDomain p j) := rfl
  have h5 : rawFive p = rawIntegral 3 (fiveDomain p) := rfl
  have h6 : rawSix p = rawIntegral 4 (sixDomain p) := rfl
  ...
  simp only [Matrix.cons_val_zero, Matrix.cons_val_succ, Fin.sum_univ_zero, add_zero,
    rawTriple, rawFour, rawFive, rawSix]
  simp only [ht, hf, h5, h6]
  norm_num
  ring
