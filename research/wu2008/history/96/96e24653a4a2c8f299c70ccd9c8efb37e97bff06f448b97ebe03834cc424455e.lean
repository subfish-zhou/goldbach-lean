...
theorem geometry (j : Fin 4) :
  ...
  fin_cases j <;> norm_num [index, Fin.natAdd, psiNode, psiTop, psiLeft, psiRight,
...
theorem theta_total_mass {δ : ℝ} (hd : 0 < δ) (hh : δ ≤ 1 / 100) :
  ...
  have hweighted : (∑ p ∈ psiPrimes (index j) N,
    ...
    calc
      _ ≤ ∑ p ∈ psiPrimes (index j) N, 6 * (1 / ((p : ℝ) - 2)) := by
        ...
        calc
          _ = (1 / ((p : ℝ) - 2)) * (1 / ((1 / 2 - δ) - log p / log N)) := by
  ...
  change boxTheta N ((N : ℝ) ^ (1 / 2 - δ))
    (fun _ : Fin 1 => psiPrimes (index j) N) ≤ _
  rw [SingleUpperNormalization.theta_single_exact (δ := δ) hN2 (psiPrimes (index j) N)
