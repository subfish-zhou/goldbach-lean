...
theorem polePrimitive_derivative (n : ℕ) (q x : ℝ) (hx : q + x ≠ 0) :
  ...
  induction n with
  | zero =>
    simpa only [polePrimitive_zero, pow_zero, one_mul] using
  ...
  | succ n ih =>
    ...
    rw [hfun]
    convert! ((((hasDerivAt_id x).pow (n + 1)).div_const ((n + 1 : ℕ) : ℝ)).sub
      (ih.const_mul q) using 1
...
theorem weightedMonomial_derivative (n : ℕ) {t : ℝ} (ht : t ∈ Icc (c 5) (c 4)) :
  ...
  have hd : HasDerivAt middleCoordinate (1 / a) t := by
    ...
    dsimp
    ring
  convert! (((polePrimitive_derivative n middlePole (middleCoordinate t) hq).comp t hd).
    const_mul 16).add
  ...
  have he : SharedRationalEnvelope.weight t =
    ...
    unfold SharedRationalEnvelope.weight middlePole middleCoordinate c u
