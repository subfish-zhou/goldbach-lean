...
theorem gamma78Gain_ratio_coordinates {R p q : ℝ} (hR : 1 < R) (hp : 1 < p) (hq : 0 < q) :
    log (R/(p*q)) / log p = gamma78GainV (log p/log R) (log q/log R) := by
  have hR0 : 0 < R := by linarith
  have hp0 : 0 < p := by linarith
  rw [log_div hR0.ne' (mul_pos hp0 hq).ne', log_mul hp0.ne' hq.ne']
  unfold gamma78GainV
  field_simp [(log_pos hR).ne', (log_pos hp).ne']
  ring

theorem gamma78Gain_cutoff_le {R p q s : ℝ}
    (hR : 1 < R) (hp : 1 < p) (hq : 0 < q) (hs : 0 < s)
    (hv : gamma78GainV (log p/log R) (log q/log R) ≤ s) :
    (R/(p*q))^(1/s) ≤ p := by
  have hL : 0 < R/(p*q) := div_pos (by linarith) (mul_pos (by linarith) hq)
  rw [← gamma78Gain_ratio_coordinates hR hp hq] at hv
  have hh := (div_le_iff₀ (log_pos hp)).mp hv
  apply (log_le_log_iff (rpow_pos_of_pos hL _) (by linarith)).mp
  rw [log_rpow hL, one_div_mul_eq_div]
  exact (div_le_iff₀ hs).mpr (by linarith)

theorem gamma78Gain_cell_count_le_phi {i k N : ℕ} {δ Δ P Q s : ℝ}
  ...
  rw [gamma5Classical_source_count_eq hp hq le_rfl (by exact_mod_cast hpq.le)]
  exact gamma5Classical_source_count_antitone _ _ _ hc

theorem gamma78Gain_grid_comparison (k : ℕ) {δ η : ℝ}
  ...
  have hp := gamma78Gain_cell_count_le_phi (by omega) hδ (by linarith) hb
