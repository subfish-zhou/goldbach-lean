...
theorem sum_shear_preserving :
    MeasurePreserving sumShear ((volume : Measure ℝ).prod volume)
      ((volume : Measure ℝ).prod volume) := by
  have h := measurePreserving_prod_neg_add_swap (volume : Measure ℝ) (volume : Measure ℝ)
  simpa only [sumShear, sub_eq_add_neg, add_comm] using h
...
theorem sum_inner_integral {z : ℝ} (hz : z ∈ Icc (2*a) (2*b)) :
  ...
  have he : (∫ x in sliceLower z..(z/2), sumKernel (z,x)) =
    ...
    rw [ho]
    simp only [truncatedSixthLowerS, truncatedSixthLowerC, sub_zero, Prod.fst, Prod.snd,
      show (1/2 : ℝ)-x-(z-x)=1/2-z by ring, a, div_eq_mul_inv, mul_inv_rev]
    ring
