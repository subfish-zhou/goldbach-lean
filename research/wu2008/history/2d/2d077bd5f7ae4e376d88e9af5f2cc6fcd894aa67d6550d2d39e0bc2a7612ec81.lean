...
theorem gamma5Gain_mesh_eventually (k : ℕ) {δ ε : ℝ}
  ...
  have hlR : δ ^ (k + 1) * log (N : ℝ) ≤ log (gamma5GainScale N δ V) := by
    ...
    simpa only [gamma5GainScale, log_rpow (by linarith : (0 : ℝ) < N)] using h
...
theorem gamma5Gain_coordinate_drift {i k N p : ℕ} {δ Δ : ℝ} {V : Fin i → ℝ}
  ...
  have hr := reboxing_support_level_bounds (Q := (N : ℝ) ^ (1 / 2 - δ))
    (rpow_nonneg (Nat.cast_nonneg N) _)
  ...
  have he : gamma5MassCoordinate (gamma5GainScale N δ V) p -
    ...
    have halg (a b c : ℝ) (hb : b ≠ 0) (hc : c ≠ 0) :
        a / b - a / c = (a / c) * ((c - b) / b) := by
      field_simp
    exact halg _ _ _ hL0.ne' hL.ne'
  rw [he]
  constructor
