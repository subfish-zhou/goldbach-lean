...
theorem raw_source_finite_floor
  ...
  (raw_source_finite_with_exception p hp (by omega) hδ hδhi hb).trans
    (add_le_add_right (add_le_add_left
      (exceptionWeighted_le_floor p δ _ hN he) _) _)
...
theorem prime_word_witness (l : List ℕ) {s t : ℕ} {a b c e f : ℝ} {cs : List ℕ}
  ...
  apply prefixTerm_one_le_of_witness (l := l ++ [s, t]) (ell := 3) _ hc
...
theorem gamma20_squarefree_discrepancy {p q r s t : ℕ}
  ...
  · rw [exceptionGamma20_unit_full]
    ...
    · have hp3' : (3 : ℝ) < p := by exact_mod_cast hp3
      ...
      simp only [List.append_cons, List.append_nil, List.map_cons, List.map_nil,
        secondFunctionalMotherColour]
