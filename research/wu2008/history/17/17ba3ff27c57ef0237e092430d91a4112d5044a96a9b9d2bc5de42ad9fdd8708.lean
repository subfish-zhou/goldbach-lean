...
theorem surplus_ftc :
  ...
  have hd (t : ℝ) : HasDerivAt
    ...
    convert (((((hasDerivAt_const t (1 / 2 : ℝ)).sub (hasDerivAt_id t)).div_const a).sub_const 3).pow 6
      |>.const_mul (-(2 / 9) * a ^ 2)) using 1 <;>
      first | rfl | (dsimp [surplusDensity, u]; field_simp [truncatedSixthLower_parameters.1.ne']; ring)
