...
theorem c6_regular_eq {t y : ℝ} (ht : t ∈ Icc alpha beta)
  ...
  · rw [if_pos hr] at h
    simpa only [truncatedSixthLowerS,truncatedSixthLowerC,sub_zero,kernel,u,
      alpha,truncatedSixthLowerAlpha] using h
  · rw [if_neg hr] at h
...
theorem c6_actual_outer_integrable :
  ...
  intro t ht
  rw [uIoc_of_le envelope_bounds.1] at ht
  have hi : (∫ y in beta..(1/2-3*alpha), truncatedSixthZeroDeltaRegular 0 (t,y)) =
    ...
    exact c6_regular_eq ⟨ht.1.le,ht.2⟩ hy
  dsimp only
