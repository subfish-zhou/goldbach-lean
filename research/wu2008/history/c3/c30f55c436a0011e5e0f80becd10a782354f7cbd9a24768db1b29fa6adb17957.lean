...
theorem paid_actual {δ : ℝ} (hd : 0 < δ) (hh : δ ≤ 1 / 10)
  ...
  exact (add_le_add_left (hp.trans hm) _).trans ha
...
theorem terminal_jMatrix_zero (k : Fin 9) : jMatrix 8 k = 0 := by
  simpa [jMatrix, assembleJ, firstNode, firstS] using
    congrArg (fun x : ℝ => x / 2) (terminal_coefficient_zero k)

theorem terminal_rationalJMatrix_zero (k : Fin 9) : rationalJMatrix 8 k = 0 := by
  simpa [rationalJMatrix, assembleJ, firstNode, firstS] using
    congrArg (fun x : ℝ => x / 2) (rational_terminal_zero k)
