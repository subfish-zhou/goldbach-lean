import MathlibNt.AnalyticNumberTheory.LargeSieve.ChenLiuCoprimeProducerVerticalIntegral

/-!
# Perron integration with independently selected primitive-character endpoints

The endpoint power is bounded before summing the integrands. There is no
interchange of a maximum with a source-coefficient sum.
-/

noncomputable section

open Classical Complex Finset MeasureTheory
open MathlibNt.SieveTheory.LiuWeight
open AnalyticNumberTheory.LargeSieve
open AnalyticNumberTheory.LargeSieve.ChenLiuCoprimeProducer
open scoped BigOperators Interval

namespace Wu2004MeanValue

/-- A selected positive endpoint for each primitive character costs the same
single Perron logarithm as a common endpoint. No positivity of the moduli is
needed for this purely analytic inequality. -/
theorem primitive_integral_selected_mean_le (S : Finset ℕ)
    (G : (q : ℕ) → PrimitiveCharacter q → ℝ → ℂ)
    (U : (q : ℕ) → PrimitiveCharacter q → ℝ)
    {σ Z T M : ℝ} (hσ : 1 / 2 ≤ σ) (hZ : 0 < Z) (hT : 0 ≤ T)
    (hM : 0 ≤ M) (hG : ∀ q ∈ S, ∀ χ, Continuous (G q χ))
    (hU : ∀ q ∈ S, ∀ χ, 0 < U q χ ∧ U q χ ≤ Z)
    (hmean : ∀ t ∈ Set.Icc (-T) T,
      (∑ q ∈ S, (q.totient : ℝ)⁻¹ * ∑ χ : PrimitiveCharacter q, ‖G q χ t‖) ≤ M) :
    (∑ q ∈ S, (q.totient : ℝ)⁻¹ * ∑ χ : PrimitiveCharacter q,
      ‖((2 * Real.pi : ℝ) : ℂ)⁻¹ *
        ∫ t in -T..T, G q χ t * (U q χ : ℂ) ^ (liuPanPerronLine σ t) /
          liuPanPerronLine σ t‖) ≤
      6 * Z ^ σ * M * Real.log (1 + T) := by
  let K : ℝ → ℝ := fun t => Z ^ σ / ‖liuPanPerronLine σ t‖
  let f : (q : ℕ) → PrimitiveCharacter q → ℝ → ℝ :=
    fun q χ t => K t * ‖G q χ t‖
  have hline : Continuous (liuPanPerronLine σ) := by
    unfold liuPanPerronLine
    fun_prop
  have hne (t : ℝ) : liuPanPerronLine σ t ≠ 0 := by
    intro h
    have := congrArg Complex.re h
    simp [liuPanPerronLine] at this
    linarith
  have hK : Continuous K :=
    continuous_const.div hline.norm (fun t => norm_ne_zero_iff.mpr (hne t))
  have hf (q : ℕ) (hq : q ∈ S) (χ : PrimitiveCharacter q) : Continuous (f q χ) :=
    hK.mul (hG q hq χ).norm
  have hfi (q : ℕ) (hq : q ∈ S) (χ : PrimitiveCharacter q) :
      IntervalIntegrable (f q χ) volume (-T) T := (hf q hq χ).intervalIntegrable _ _
  have hnorm : ‖((2 * Real.pi : ℝ) : ℂ)⁻¹‖ ≤ 1 := by
    rw [norm_inv, Complex.norm_real, Real.norm_eq_abs, abs_of_pos (by positivity)]
    apply (inv_le_one₀ (by positivity)).mpr
    linarith [Real.pi_gt_three]
  have hpiece (q : ℕ) (hq : q ∈ S) (χ : PrimitiveCharacter q) :
      ‖((2 * Real.pi : ℝ) : ℂ)⁻¹ *
        ∫ t in -T..T, G q χ t * (U q χ : ℂ) ^ (liuPanPerronLine σ t) /
          liuPanPerronLine σ t‖ ≤ ∫ t in -T..T, f q χ t := by
    rw [norm_mul]
    calc
      _ ≤ ‖∫ t in -T..T, G q χ t * (U q χ : ℂ) ^ (liuPanPerronLine σ t) /
          liuPanPerronLine σ t‖ :=
        mul_le_of_le_one_left (norm_nonneg _) hnorm
      _ ≤ ∫ t in -T..T, f q χ t := by
        refine intervalIntegral.norm_integral_le_of_norm_le (by linarith)
          (Filter.Eventually.of_forall ?_) (hfi q hq χ)
        intro t _
        simp only [norm_div, norm_mul,
          Complex.norm_cpow_eq_rpow_re_of_pos (hU q hq χ).1]
        have hre : (liuPanPerronLine σ t).re = σ := by simp [liuPanPerronLine]
        rw [hre]
        calc
          _ ≤ ‖G q χ t‖ * Z ^ σ / ‖liuPanPerronLine σ t‖ := by
            exact div_le_div_of_nonneg_right
              (mul_le_mul_of_nonneg_left
                (Real.rpow_le_rpow (hU q hq χ).1.le (hU q hq χ).2 (by linarith))
                (norm_nonneg _)) (norm_nonneg _)
          _ = _ := by dsimp [f, K]; ring
  have hsumc (q : ℕ) (hq : q ∈ S) :
      Continuous (fun t => ∑ χ : PrimitiveCharacter q, f q χ t) := by
    fun_prop
  have htotalc : Continuous
      (fun t => ∑ q ∈ S, (q.totient : ℝ)⁻¹ * ∑ χ : PrimitiveCharacter q, f q χ t) := by
    apply continuous_finsetSum
    intro q hq
    exact continuous_const.mul (hsumc q hq)
  calc
    _ ≤ ∑ q ∈ S, (q.totient : ℝ)⁻¹ * ∑ χ : PrimitiveCharacter q,
        ∫ t in -T..T, f q χ t := by
      apply sum_le_sum
      intro q hq
      apply mul_le_mul_of_nonneg_left _ (by positivity)
      exact sum_le_sum fun χ _ => hpiece q hq χ
    _ = ∫ t in -T..T,
        ∑ q ∈ S, (q.totient : ℝ)⁻¹ * ∑ χ : PrimitiveCharacter q, f q χ t := by
      rw [intervalIntegral.integral_finsetSum
        (f := fun q t => (q.totient : ℝ)⁻¹ * ∑ χ : PrimitiveCharacter q, f q χ t)
        (fun q hq => (continuous_const.mul (hsumc q hq)).intervalIntegrable _ _)]
      apply sum_congr rfl
      intro q hq
      rw [intervalIntegral.integral_const_mul,
        intervalIntegral.integral_finsetSum (fun χ _ => hfi q hq χ)]
    _ ≤ ∫ t in -T..T, K t * M := by
      apply intervalIntegral.integral_mono_on (by linarith)
        (htotalc.intervalIntegrable _ _) ((hK.mul continuous_const).intervalIntegrable _ _)
      intro t ht
      calc
        _ = K t * (∑ q ∈ S, (q.totient : ℝ)⁻¹ *
            ∑ χ : PrimitiveCharacter q, ‖G q χ t‖) := by
          simp only [f, mul_sum]
          apply sum_congr rfl
          intro q _
          apply sum_congr rfl
          intro χ _
          ring
        _ ≤ _ := mul_le_mul_of_nonneg_left (hmean t ht) (by dsimp [K]; positivity)
    _ = (Z ^ σ * M) * (∫ t in -T..T, 1 / ‖liuPanPerronLine σ t‖) := by
      rw [← intervalIntegral.integral_const_mul]
      apply intervalIntegral.integral_congr
      intro t _
      dsimp [K]
      ring
    _ ≤ (Z ^ σ * M) * (6 * Real.log (1 + T)) :=
      mul_le_mul_of_nonneg_left (vertical_reciprocal_integral_le hσ hT) (by positivity)
    _ = _ := by ring

end Wu2004MeanValue
