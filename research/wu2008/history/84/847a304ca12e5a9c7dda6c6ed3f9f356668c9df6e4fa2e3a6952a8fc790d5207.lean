...
elab "#w02_parent_cone" : command => do
  let env := (← getEnv).setExporting false
  for root in #[`WuTarget.W02.output_lower, `WuTarget.W02.certified_twentyone_actual,
      `WuTarget.W02Accepted.nodeGain, `WuTarget.W02Accepted.enhanced_P2_paid] do
    let mut todo := #[root]
    ...
    let forbidden := names.filter fun n =>
      n == "Lean.ofReduceBool" || n == "Lean.trustCompiler" ||
