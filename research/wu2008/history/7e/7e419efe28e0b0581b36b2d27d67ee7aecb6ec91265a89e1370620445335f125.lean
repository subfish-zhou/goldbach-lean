...
elab "#w04_parent_cone" : command => do
  let env := (← getEnv).setExporting false
  for root in #[`WuTarget.W04.extraTable_le_kernel,
      `WuTarget.W04.elementary_add_extra_le_feedbackMatrix,
      `WuTarget.W04.augmented_same_delta, `WuTarget.W04.augmented_paid_update] do
    let mut todo := #[root]
    ...
    let forbidden := names.filter fun n =>
      n.endsWith ".Ainf" || n.endsWith ".originalH" ||
