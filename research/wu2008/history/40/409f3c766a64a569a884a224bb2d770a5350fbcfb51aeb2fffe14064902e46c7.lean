...
theorem fibre_eq_uniform_integral (j : Fin 21) {x : ℝ} (hx : x ∈ Icc alpha beta) :
  ...
  have hs : Function.support (fun y => uniform (basis j) 0 (x,y)) ⊆
    ...
    intro y hy
    by_contra hn
    have hd : (x,y) ∉ StaircaseShrink.domain 0 := fun hd => hn hd.2
    exact hy (if_neg hd)
...
theorem fibre_clipped (j : Fin 21) {x : ℝ} (hx : x ∈ Icc alpha beta) :
  ...
  · rw [if_pos h, truncatedSixthMass_integral_eq_interval h (uniform_slice_support j x)]
    ...
    change uniform (basis j) 0 (x,y) = reciprocalKernel x y
...
theorem logPrimitive_derivative {x y : ℝ}
  ...
  have hh := (h1.sub h2).div_const (x*(1/2-x))
  change HasDerivAt (logPrimitive x)
    ((1/y - (0-1)/(1/2-x-y))/(x*(1/2-x))) y at hh
  have he : (1/y - (0-1)/(1/2-x-y))/(x*(1/2-x)) = reciprocalKernel x y := by
    unfold reciprocalKernel
    have halg {c : ℝ} (hcn : c ≠ 0) (hyn : c-y ≠ 0) :
        (1/y - (0-1)/(c-y))/(x*c) = 1/(x*y*(c-y)) := by
      field_simp
      <;> ring
    exact halg hc.ne' hz.ne'
  rw [he] at hh
  exact hh
