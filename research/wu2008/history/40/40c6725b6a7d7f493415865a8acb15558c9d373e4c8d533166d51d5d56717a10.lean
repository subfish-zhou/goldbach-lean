...
noncomputable def gamma78GainInnerRectangle (tri : Bool) (n : ℕ)
    (j : {j // j ∈ gamma78GainInner tri n}) : Gamma78GainRectangle tri := by
  obtain ⟨_,ha,hb,hbc,hc,hd,hs⟩ := mem_filter.mp j.property
  have hab := truncatedSixthClosure_lo_lt_hi n j.val.1
  have hcd := truncatedSixthClosure_lo_lt_hi n j.val.2
  have hD : truncatedSixthClosureHi n j.val.2 < gamma5ClassicalB :=
    hd.trans_le (gamma78Gain_constants tri).2.2.2.2.2
  refine ⟨⟨truncatedSixthClosureLo n j.val.1,truncatedSixthClosureHi n j.val.1,
    truncatedSixthClosureLo n j.val.2,truncatedSixthClosureHi n j.val.2,5/2,
    ha,hab,hbc,hcd,hD,?_,?_,by norm_num⟩,hb,hc,hd,gamma78GainSample n j.val,?_,hs⟩
  · norm_num [gamma6BaseB,gamma5ClassicalB] at hb hD
    linarith
  · have hca : gamma5MassA < truncatedSixthClosureLo n j.val.2 :=
      ha.trans (hab.trans hbc)
    dsimp [gamma5GainV]
