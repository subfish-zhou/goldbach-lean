...
theorem coupled_RHS_analytic_prefix (j : Fin 3) {δ ε : ℝ}
  ...
  obtain ⟨T1, _, hT1⟩ := omega2_integral_paid
    ...
    (by rw [← hs]; exact (seven_parameter_geometry (j.castAdd 4)).2.2.2.2.1) hd hh he
...
theorem coupled_allowance_identity (j : Fin 3) {δ : ℝ} (hh : δ ≤ 1 / 100) :
  ...
  rw [hA] at hbase
  have hconstant : coupledLinear (Wu04RemainingCore.row j) +
      SecondFunctionalCoupledFeedback.classical (Wu04RemainingCore.row j) =
      5 - Wu08OriginalPsiRecovery.classicalNumerator (Wu04RemainingCore.row j) := by
    unfold coupledLinear
    linarith only [hbase]
  unfold gammaAllowance
  rw [← add_assoc, hconstant]
  unfold coupledForcing
