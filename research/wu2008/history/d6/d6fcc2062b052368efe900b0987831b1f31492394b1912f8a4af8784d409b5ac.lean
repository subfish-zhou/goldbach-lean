...
def paidCoefficient : ℝ := W17Accepted.paidCoefficient+netCredit
def remainingCoefficient : ℝ := W17Accepted.remainingCoefficient+netCredit

theorem paid_exact : paidCoefficient =
    W11CreditAccepted.paidCoefficient+(original10+original11-pairUpper)/4+
      W17Accepted.jointCredit := by
  unfold paidCoefficient W17Accepted.paidCoefficient
  rw [W13TightAccepted.paid_exact,net_identity]
  ring

theorem paid_gain_identity :
    paidCoefficient-W17Accepted.paidCoefficient =
      (W13Tight.pairUpper-pairUpper)/4 := by
  rw [exact_recovery]
  unfold paidCoefficient netCredit
  ring

theorem paid_gain :
    W17Accepted.paidCoefficient+999/200000 < paidCoefficient := by
  unfold paidCoefficient
  linarith only [netCredit_bounds.1]
