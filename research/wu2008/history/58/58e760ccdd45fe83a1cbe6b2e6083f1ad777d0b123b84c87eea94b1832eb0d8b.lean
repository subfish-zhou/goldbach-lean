...
theorem paper_eq_rectangles (p : ℝ → ℝ) : paperG6 p = rectangleG6 p := by
  ...
  simp_rw [show ∀ t : ℝ, (1/2-t-beta)/alpha=(1/2-beta-t)/alpha by
    intro t; ring] at hA
  have hAi := intervalIntegral.integral_congr (a := alpha) (b := beta) (fun t _ => hA t)
  have hBi := intervalIntegral.integral_congr (a := alpha) (b := 3*alpha/2) (fun t _ => hB t)
...
theorem profile_actual {w : Fin 21 → ℝ} {delta s : ℝ}
  ...
  apply StaircaseShrink.eval_le_of_nodes (rows w) _ hm
    (fun r hr => ?_) ⟨hs.1,hs.2.trans hse⟩ le_rfl
    (wuImprovementLimit_nonneg false hd (by linarith) (by linarith [hs.1])
      (by linarith [hs.2]))
  obtain ⟨j,_,rfl⟩ := List.mem_map.mp hr
  exact ⟨row_endpoint w (List.mem_map.mpr ⟨j,List.mem_finRange j,rfl⟩),
    by change w j ≤ wuImprovementLimit false delta (originalRow j).2.1
       rw [originalRow_node]
       exact hw j⟩
