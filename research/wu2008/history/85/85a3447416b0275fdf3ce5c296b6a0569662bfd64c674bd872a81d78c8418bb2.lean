...
theorem source_fixed_inner_change (H : ℝ → ℝ) {S t a b : ℝ} (hS : S ≠ 0) :
  ...
  convert rfl using 1 <;> congr 1 <;> ring
...
theorem source66_change_of_variables (H : ℝ → ℝ)
  ...
  dsimp only
  congr 1
  have h := source_fixed_inner_change H (S := p.S) (t := t)
    (a := t) (b := 1 / p.kappa2) hS
  convert h using 1 <;>
    first | (congr 1 <;> ring) |
      (apply intervalIntegral.integral_congr; intro u _; congr 2; ring)
