...
noncomputable def gamma6BasePrimes (N : ℕ) (R A B cap : ℝ) : Finset ℕ :=
  ((gamma5MassPrimes R A B).filter (fun p => (p : ℝ) < R ^ cap)).filter
    (fun p => p.Coprime N)
...
theorem gamma6Base_upper_atom {R A B cap Z M : ℝ}
    ...
    |(∑ p ∈ gamma5MassPrimes R A B, g p) -
      ∑ p ∈ (gamma5MassPrimes R A B).filter (fun p => (p : ℝ) < R ^ cap), g p| ≤ M / Z := by
  let P := gamma5MassPrimes R A B
  let E := P.filter (fun p => ¬(p : ℝ) < R ^ cap)
  ...
  have hcard : E.card ≤ 1 := (card_le_card hsub).trans (by simp)
  have he : (∑ p ∈ P, g p) - ∑ p ∈ P.filter (fun p => (p : ℝ) < R ^ cap), g p =
      ∑ p ∈ E, g p := by
    have h := sum_filter_add_sum_filter_not P (fun p => (p : ℝ) < R ^ cap) g
    dsimp [E]
    linarith
  change |(∑ p ∈ P, g p) - _| ≤ _
  rw [he]
...
theorem gamma6Base_window_discrepancy {N : ℕ} {R A B cap α M e : ℝ}
  ...
  let P := gamma5MassPrimes R A B
  let P' := P.filter (fun p => (p : ℝ) < R ^ cap)
