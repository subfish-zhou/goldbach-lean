...
/-! Actual large-source AP distribution. This consumes both analytic
primitive producers and the true principal remainder, with the two-log
cofactor payment and no Wu-weight assumption on primitive conductors. -/
...
theorem large_actualAP_unweighted (A : ℝ) (hA : 0 < A) :
  ...
  obtain ⟨b, J, hb, hJ, X₁, hprimitive⟩ :=
    commonPrimitiveSource_log_saving_uniform_level (A + 2) (by linarith)
  obtain ⟨P, hP, X₂, hprincipal⟩ :=
    principal_selected_unweighted_log_saving A 1 1 hA (by norm_num)
  ...
  have hprim (h : ℕ) (hh : h ∈ Icc 1 Q) :
    ...
    apply hprimitive x hx₁ B hB h Q L U (fun m => (f m : ℂ)) r
      (mem_Icc.mp hh).1
      ((by exact_mod_cast (mem_Icc.mp hh).2 : (h : ℝ) ≤ Q).trans hQroot)
      hU hQ hL
    · intro m
  ...
  have hprincipal' := hprincipal x hx₂ Q (fun _ => Ioc L U) (fun _ => f) (fun _ => r)
    hQx (fun _ _ => hS) (fun _ _ m _ => hf m)
    (fun _ _ m hm => by simpa only [one_mul] using hr m hm)
  calc
    _ ≤ (∑ d ∈ Icc 1 Q,
