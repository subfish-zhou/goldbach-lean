...
theorem weighted_tail_exchange {a b B : ℝ} (hab : a ≤ b) (hbB : b ≤ B)
  ...
  constructor
  · exact hi.congr (fun x hx => (he x hx).symm)
...
theorem rationalWeight_rescale {a b c : ℝ} (ha : 0 < a) (hab : a < b)
  ...
  simp only [smul_eq_mul, ← mul_assoc, mul_inv_cancel₀ hc.ne', one_mul, mul_comm c]
...
theorem rationalWeight_partial_log {a b c u : ℝ} (ha : 0 < a) (hab : a < b)
