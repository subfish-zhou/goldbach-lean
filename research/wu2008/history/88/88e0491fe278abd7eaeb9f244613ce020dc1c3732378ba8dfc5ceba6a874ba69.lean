...
theorem sourceCoefficient_first (j : ℕ) :
  ...
  simp only [sourceCoefficient, coefficientLeft, if_pos rfl, upperKernel]
...
theorem coefficient_integral_partition {f : ℝ → ℝ}
  ...
  rw [Finset.sum_eq_add_sum_diff_singleton (Finset.mem_Icc.mpr
    ⟨le_rfl, by omega⟩ : 2 ∈ Finset.Icc 2 10)]
  have hset : Finset.Icc 2 10 \ {2} = Finset.Icc 3 10 := by
    ext i
  ...
  rw [he, hsum, h10]
  simp only [coefficientLeft, if_pos rfl]
