...
theorem closed_indicator_split {a b c t : ℝ} (hab : a ≤ b) (hbc : b ≤ c)
  ...
  by_cases hta : a ≤ t <;> by_cases htb : t ≤ b <;> by_cases htc : t ≤ c <;>
    simp_all only [indicator, mem_Icc, not_le]
  all_goals split_ifs <;> first | rfl | norm_num | grind
...
theorem K65_integral_identity {f : ℝ → ℝ} (hf : IntervalIntegrable f volume 1 3)
  ...
  refine ⟨hi.congr_ae (by filter_upwards [he] with t ht; exact fun hm => (ht hm).symm), ?_⟩
