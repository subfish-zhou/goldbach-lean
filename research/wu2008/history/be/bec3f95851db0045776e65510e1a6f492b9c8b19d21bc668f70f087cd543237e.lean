...
theorem xi2_product_factors_positive (i : Fin 4) :
  ...
  fin_cases i <;>
    norm_num [xi2Right, alpha5, coupledRow, SecondFunctionalPositive.parameters,
      SecondFunctionalParameters.row1, SecondFunctionalParameters.row2,
      SecondFunctionalParameters.row3, SecondFunctionalParameters.row4]
...
theorem xi2_denominator_positive (i : Fin 4) (j : Fin 9) {t : ℝ}
  ...
  fin_cases j
  · exact le_rfl
  · exact mul_le_mul_of_nonneg_left (by linarith) hk2
  · exact mul_le_mul_of_nonneg_left (by linarith) hk3
  · exact mul_le_mul_of_nonneg_left (by linarith) hs
  · exact le_rfl
  · have hp := xi2_product_factors_positive i
    change _ * _ ≤ _ * _
    exact mul_le_mul (by nlinarith) (by nlinarith)
      hp.2.le (by nlinarith [hp.1])
  · change _ - _ - _ ≤ _ - _ - _
    nlinarith
  · change _ - _ - _ ≤ _ - _ - _
    nlinarith
