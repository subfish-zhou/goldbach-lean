...
theorem qLeft_cast (k : Fin 9) : (qLeft k : ℝ) = upperLeft k := by
  simp [qLeft, upperLeft]

theorem qTail_cast (S : ℚ) (k : Fin 9) : (qTail S k : ℝ) = tailCell S k := by
  unfold qTail tailCell tailLeft Wu04Bypass.cell
  simp only [Rat.cast_ite, Rat.cast_zero, Rat.cast_div, Rat.cast_mul, Rat.cast_sub,
    Rat.cast_add, Rat.cast_ofNat, Rat.cast_one, Rat.cast_max, qUpper_cast, qLeft_cast]
