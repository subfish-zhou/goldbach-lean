...
theorem fixed_cutoff_eq {R P Q S : ℝ}
  ...
  congr 1
  calc
    _ = (log R / S) * ((1 - log P / log R - log Q / log R) /
        (1 - log P / log R - log Q / log R)) := by
      simp only [div_eq_mul_inv, mul_inv_rev]
      ring
    _ = _ := by rw [div_self hb, mul_one]; ring
...
theorem full_count_exact_phi_atoms {p : SecondFunctionalParameters}
  ...
  have hid := termCount_eq_original p hp.toAnalyticParameters .gammaFive hN hδ hδhi hb
  simp only [Term.index] at hid
  rw [← hid, termCount_eq_sum]
  apply sum_congr rfl
  intro x hx
...
theorem two_endpoint_prefixes {R P Q : ℝ} (hP : 0 < P) (hQ : 0 < Q) :
  ...
  have he : (![Q, P] : Fin 2 → ℝ) =
      Fin.cons Q (Fin.cons P (Fin.elim0 : Fin 0 → ℝ)) := by
    ext j
    fin_cases j <;> rfl
  rw [he, boxSquaredPrefixes_cons_iff hQ, boxSquaredPrefixes_cons_iff hP]
  have hz (L : ℝ) : boxSquaredPrefixes L (Fin.elim0 : Fin 0 → ℝ) := by
    intro j
    exact Fin.elim0 j
