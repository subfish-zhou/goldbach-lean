...
theorem density_nonneg {s : ℝ} (hs : s ∈ Icc 2 endpoint) : 0 ≤ density s := by
  ...
  let z := 1/2-alpha*s
  have hm : 0 < upperX z := alpha_pos.trans_le hg.1
  have ha : 0 < z-alpha := by linarith [hg.1,hg.2.1]
...
theorem basis_density_integral (j : Fin 21) :
  ...
  have he : (fun s => density s * profile (WuTarget.W03.basis j) s) =
    ...
    simp only [WuTarget.W03.cell,indicator,mem_Ioc]
    split_ifs <;> simp
...
theorem published_twentyone (w : Fin 21 → ℝ) :
  ...
  nlinarith only [mul_eq_mul_left (w j) h]
