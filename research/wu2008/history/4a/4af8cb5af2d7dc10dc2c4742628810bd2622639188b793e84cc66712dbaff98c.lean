...
theorem gamma5Gain_approx_integral (δ : ℝ) (n : ℕ) :
  ...
  rw [integral_finset_sum _ (fun j _ => gamma5Gain_approx_summand_integrable δ n j)]
  ...
  have heq : (truncatedSixthClosureCell n j.val).indicator
    ...
    · simp only [Set.indicator_of_notMem hv]
      change 0 = gamma5GainH δ r.s *
        (truncatedSixthClosureCell n j.val).indicator gamma5GainSmooth v
...
theorem gamma5Gain_inner_packings_disjoint {i N n : ℕ} {δ Δ : ℝ} {V : Fin i → ℝ}
  ...
  have heq : j.val = l.val :=
    truncatedSixthClosure_cell_unique
