...
theorem caseII_total_le_doubleRounded_direct_concrete_relative_natCeil
  ...
  have hSharp :=
    caseII_total_le_from_caseI_endpoint_explicit_rawBase_natCeil
      (S := S) (H := H) (N := N) (D := D) (y := y) (z := z)
      (σ := σ) (C := C) (C1 := C1) (K := K) (ΘK := ΘK)
      (Δ := Δ) (d := d) (B0 := B0) (s := s)
      hH hN hN2 hycube hyceil hyDhalf h3σ hD hDlarge hwy hy2 hyr2 hw2
      hEndpoint hnu hC hlog hSourceDomain hClaim14_6_i hErrorDomain hIH
