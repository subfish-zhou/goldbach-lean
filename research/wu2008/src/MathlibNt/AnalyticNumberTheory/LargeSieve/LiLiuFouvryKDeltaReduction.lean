import MathlibNt.AnalyticNumberTheory.LargeSieve.LiLiuFouvryKHighOmegaReduction
...
theorem signedError_sq_le_lowOmega_twoGCD_truncated_kscale
    ...
    ∀ᶠ x : ℝ in atTop, ∀ z : ι, ∀ M L : ℝ,
      ...
      signedError S (N z) Q α (β z) c a ^ 2 ≤
        4 * (∑ m ∈ S, α m ^ 2) *
          ...
          x ^ 2 / Real.log x ^ A := by
  filter_upwards [
    signedError_sq_le_lowOmega_firstGCD_truncated_kscale
      (i := i) (j := j) (A + 2) hSW hT hN hβ hCscale hε hη,
    betaClean_wMaskedTruncated_largeDelta_dyadic_kscale i k j (A + 2) hε hη hCscale,
