import MathlibNt.AnalyticNumberTheory.LargeSieve.LiLiuFouvryKSecondaryWeight
...
after the fixed Cscale but before x, all varying scales, the family, the signed shift, keys and dyadic data. -/
theorem directJoinedKSecondary_uniform
  ...
  obtain ⟨Cw, hCw, hweight⟩ := directPayKSecondary_weight_paid
    (ρ := ρ) (Ccoeff := Ccoeff) hscale hκ hδ hC hCsec
  ...
  obtain ⟨hHg, hAg, hqg⟩ := directPayKSecondary_growth_data hscale hx hη hη1 hM hT hxMT
