import MathlibNt.AnalyticNumberTheory.LargeSieve.LiLiuPrereqFouvryWModulusSupportPayment

/-!
# Supported-modulus exclusions at the actual C.2 scale

For `x = 4MT`, `T ≤ x^(1/10)` and `L ≤ x^(5/9)` imply `L ≤ M`
...
theorem betaClean_wMaskedTruncated_modulus_support_dyadic
  ...
  let u : ℕ := (i + 1) ^ 2 - 1
  filter_upwards [
    eventually_wMaskedOriginal_zero_modulus_support_power_saving
      (by omega : 1 ≤ k + 1) j hη,
  ...
  have hLM : L ≤ M := by
    ...
    have hbound : 4 * T * x ^ (5 / 9 : ℝ) ≤ 4 * T * M := by
      calc
        _ ≤ 4 * x ^ (1 / 10 : ℝ) * x ^ (5 / 9 : ℝ) := by gcongr
        _ = 4 * x ^ ((1 / 10 : ℝ) + 5 / 9) := by
