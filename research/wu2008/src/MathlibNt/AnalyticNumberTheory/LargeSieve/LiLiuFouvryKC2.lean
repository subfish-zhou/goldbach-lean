import MathlibNt.AnalyticNumberTheory.LargeSieve.LiLiuFouvryKPrefixReduction
import MathlibNt.AnalyticNumberTheory.LargeSieve.LiLiuFouvryKTotalCost
import MathlibNt.AnalyticNumberTheory.LargeSieve.LiLiuPrereqFouvryDirectC2
noncomputable section
open Classical Finset Filter
namespace MathlibNt.AnalyticNumberTheory.LargeSieve.LiLiuPrereqFouvry
theorem direct_wellFactorable_signedError_sq_kscale
    ...
    ∀ᶠ x : ℝ in atTop, ∀ z : ι, ∀ M ν : ℝ,
      1 ≤ M → 4*M*T z = x → ε ≤ ν → ν ≤ 1/10+ε/10 → T z = x^ν →
  ...
  obtain ⟨C,hC,hcost⟩ := direct_total_prefix_cost_kscale i k j hCscale hε hη hη1 hηε hbudget
  have hpow := direct_power_to_log C hC.le A (show 0 < ε/4 by positivity)
  filter_upwards [hcost, hpow,
    wellFactorable_signedError_sq_le_floor_prefix_kscale (i := i) (j := j) A
      hSW hT hN hβ hCscale hε hη] with x hcostx hpowx hreduce
...
theorem direct_wellFactorable_signedError_kscale
    {ι : Type*} {κ k i j : ℕ} (A : ℕ)
    {T : ι → ℝ} {N : ι → Finset ℕ} {β : ι → ℕ → ℝ}
    (hSW : BetaCoprimeSWFamily κ T N β) (hT : ∀ z, 1 ≤ T z)
    (hN : ∀ z, ∀ n ∈ N z, T z ≤ (n : ℝ) ∧ (n : ℝ) ≤ 2*T z)
    ...
    ∀ᶠ x : ℝ in atTop, ∀ z : ι, ∀ M ν : ℝ,
      ...
      |signedError U (N z) (Ioc 0 ⌊x^((5-5*ν)/9-ε)⌋₊) α (β z) c a| ≤
        x/Real.log x^A := by
  filter_upwards [direct_wellFactorable_signedError_sq_kscale (i := i) (j := j) (2*A+2)
    hSW hT hN hβ hCscale hε, Real.tendsto_log_atTop.eventually_ge_atTop (2 : ℝ),
