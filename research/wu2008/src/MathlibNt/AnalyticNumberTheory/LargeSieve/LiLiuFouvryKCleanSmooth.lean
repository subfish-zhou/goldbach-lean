...
theorem kClean_smooth_error_scale_product_le {x M T L : ℝ}
    (hx : 0 < x) (hT : 0 ≤ T) (hL : 0 ≤ L)
    (hMT : M * T ≤ x) (hTx : T ≤ 2 * x ^ (1 / 9 : ℝ))
    (hLx : L ≤ x ^ (5 / 9 : ℝ)) :
    M * T ^ 2 * L ≤ 2 * x ^ (5 / 3 : ℝ) := by
  calc
    _ = (M * T) * T * L := by ring
    _ ≤ x * (2 * x ^ (1 / 9 : ℝ)) * x ^ (5 / 9 : ℝ) := by gcongr
    _ = 2 * (x ^ (1 : ℝ) * x ^ (1 / 9 : ℝ) * x ^ (5 / 9 : ℝ)) := by
      rw [Real.rpow_one]
      ring
    _ = 2 * x ^ (5 / 3 : ℝ) := by
...
theorem kClean_alpha_sq_mul_smooth_envelopes_le_c2
  ...
  have hT' : T ≤ 2 * x := hTx.trans (by
    gcongr
    simpa using Real.rpow_le_rpow_of_exponent_le hx (show (1 / 9 : ℝ) ≤ 1 by norm_num))
  ...
  calc
    ...
    _ ≤ _ := by
      ...
      convert mul_le_mul_of_nonneg_left
        (kClean_smooth_error_scale_product_le (by linarith)
...
theorem kClean_eventually_smooth_error_log_payment (K A : ℕ) {C : ℝ} (hC : 0 ≤ C) :
    ∀ᶠ x : ℝ in atTop,
      C * x ^ (5 / 3 : ℝ) * (1 + Real.log (2 * x)) ^ K ≤
        x ^ 2 / Real.log x ^ A := by
  have hsmall : (fun x : ℝ => Real.log x ^ (K + A)) =o[atTop]
      (fun x => x ^ (1 / 3 : ℝ)) := by
    simpa only [Real.rpow_natCast] using
      isLittleO_log_rpow_rpow_atTop (K + A : ℕ) (by norm_num : (0 : ℝ) < 1 / 3)
  ...
  have hH : 1 + Real.log (2 * x) ≤ 3 * Real.log x := by
    ...
    linarith
  have hb' : C * 3 ^ K * Real.log x ^ (K + A) ≤ x ^ (1 / 3 : ℝ) := by
  ...
  calc
    _ ≤ C * x ^ (5 / 3 : ℝ) * (3 * Real.log x) ^ K * Real.log x ^ A := by
      gcongr
    _ = x ^ (5 / 3 : ℝ) * (C * 3 ^ K * Real.log x ^ (K + A)) := by
      rw [pow_add, mul_pow]
      ring
    _ ≤ x ^ (5 / 3 : ℝ) * x ^ (1 / 3 : ℝ) :=
...
theorem kClean_alpha_sq_mul_smooth_envelopes_c2_log_payment
    {i k j : ℕ} (hi : 1 ≤ i) (hk : 1 ≤ k) (hj : 1 ≤ j)
    (A : ℕ) {C : ℝ} (hC : 0 ≤ C) :
    ∀ᶠ x : ℝ in atTop, ∀ M T L : ℝ,
      1 ≤ M → 1 ≤ T → 1 ≤ L → M * T ≤ x →
      T ≤ 2 * x ^ (1 / 9 : ℝ) → L ≤ x ^ (5 / 9 : ℝ) →
      ∀ S N Q : Finset ℕ,
      (∀ m ∈ S, M ≤ (m : ℝ) ∧ (m : ℝ) ≤ 2 * M) →
      N ⊆ Ioc 0 ⌊T⌋₊ → Q ⊆ Ioc 0 ⌊L⌋₊ →
      ∀ α β c : ℕ → ℝ,
      ...
      C * ((∑ m ∈ S, α m ^ 2) *
        (smoothUErrorEnvelope N Q β c a + smoothVErrorEnvelope N Q β c a)) ≤
          x ^ 2 / Real.log x ^ A := by
  filter_upwards [kClean_eventually_smooth_error_log_payment
  ...
  have h := mul_le_mul_of_nonneg_left
    (kClean_alpha_sq_mul_smooth_envelopes_le_c2 hi hk hj hx hM hT hL hMT hTx hLx
...
theorem kClean_dispersionUV_smooth_c2_log_payment
  ...
  filter_upwards [kClean_alpha_sq_mul_smooth_envelopes_c2_log_payment hi hk hj A hC] with x hx
...
theorem kClean_dispersionUV_smooth_c2_dyadic_log_payment
    {i k j : ℕ} (hi : 1 ≤ i) (hk : 1 ≤ k) (hj : 1 ≤ j) (A : ℕ) :
    ∀ᶠ x : ℝ in atTop, ∀ M T L : ℝ,
      1 ≤ M → 1 ≤ T → 1 ≤ L → 4 * M * T = x →
      T ≤ x ^ (1 / 9 : ℝ) → L ≤ x ^ (5 / 9 : ℝ) →
      ...
      (∑ m ∈ S, α m ^ 2) *
        (|dispersionU P N Q (fun m => scaledDyadicCutoff M m) β c a -
          ...
          2 * |dispersionV P N Q (fun m => scaledDyadicCutoff M m) β c a -
            smoothUMain M N Q β c a|) ≤ x ^ 2 / Real.log x ^ A := by
  filter_upwards [kClean_dispersionUV_smooth_c2_log_payment hi hk hj A] with x hx
