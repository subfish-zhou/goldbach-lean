import MathlibNt.AnalyticNumberTheory.LargeSieve.LiLiuFouvryKCell
import MathlibNt.AnalyticNumberTheory.LargeSieve.LiLiuFouvryExtendedPair
...
theorem direct_key_kscale (k m : ℕ) {Cscale ε η : ℝ}
    (hCscale : 1 ≤ Cscale)
  ...
  obtain ⟨C,hC,hcell⟩ := direct_retained_cell_kscale k m hCscale hε hη hη1 hηε hbudget
  ...
  have hRS := (c2_extended_factor_levels hx1 hε hεν hν).2.2.1
  let U := wExtractedKeyFiber (wFloorCutoff M (x^η)) N
    (Ioc 0 ⌊x^((5-5*ν)/9-ε)⌋₊) a (c2FiveSmallMask x η)
    (x^c2RExponent ν ε) (x^c2SExponent ν ε) (highOmegaCutoff x) b K
  have hQ : ∀ q ∈ Ioc 0 ⌊x^((5-5*ν)/9-ε)⌋₊, 0 < q := fun _ hq => (mem_Ioc.mp hq).1
  have hpair := wCoprimePairBound_extended_le hx hε hεν hν hNT
