import MathlibNt.Wu2008DoubleSieve.SixthReciprocalPayment
...
noncomputable def newBalance : ℝ :=
  BaseRecurrenceLower.newBase+UnroundedPayments.fifthRational+
    RationalMovingSixth.rationalSixth+deltaSixth+47/481250-GConvexChord.rationalG-
...
theorem exact_balance_gain : newBalance = GConvexChord.newBalance+deltaSixth := by
  unfold newBalance GConvexChord.newBalance
  ring

theorem balance_strictly_improved : GConvexChord.newBalance < newBalance := by
  rw [exact_balance_gain]
  linarith only [deltaSixth_pos]

noncomputable def exactBalance : ℝ := GConvexChord.exactBalance+
  1609168471475952604903444376820277244537532780965684646396726475412762/8973226984304932235772272497178442189932542751246851968056677532004417

theorem newBalance_eq_exact : newBalance = exactBalance := by
  rw [exact_balance_gain,GConvexChord.newBalance_eq_exact,deltaSixth_exact]
  rfl
