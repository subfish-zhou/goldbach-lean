...
/-- One threshold before N, boxes, mother parameters and the original six words. -/
...
/-- All six raw-square pieces cost a single epsilon, not six epsilons. -/
theorem source_squareRawMass_six_payment (k : ℕ) {δ ε : ℝ}
    (hδ : 0 < δ) (hδhi : δ < 1/2) (hε : 0 < ε) :
    ∃ T : ℕ, 4 ≤ T ∧ ∀ N : ℕ, T ≤ N →
      ∀ (i : ℕ) (Δ : ℝ) (V : Fin i → ℝ), wuSourceBox k δ N i Δ V →
  ...
  calc
    _ ≤ ∑ _j : Fin 6, ε/6 * boxTheta N ((N : ℝ)^(1/2-δ))
...
theorem source_bad_primeMass_payment (k : ℕ) {δ ε : ℝ}
    ...
    ∃ T : ℕ, 4 ≤ T ∧ ∀ N : ℕ, T ≤ N →
      ...
      (sourceFamily N δ Δ V p j).noncoprimePart.primeMass ≤
        ε * boxTheta N ((N : ℝ)^(1/2-δ)) (convolutionWuWindows N Δ V) := by
  let C := (max 1 (1/(wuLocalExponent k δ / 10)))^(k+3) / log 2
...
/-- The original all-six exceptional-prime sum is paid using its explicit 6G log bound. -/
theorem source_bad_primeMass_six_payment (k : ℕ) {δ ε : ℝ}
    (hδ : 0 < δ) (hδhi : δ < 1/2) (hε : 0 < ε) :
    ∃ T : ℕ, 4 ≤ T ∧ ∀ N : ℕ, T ≤ N →
      ∀ (i : ℕ) (Δ : ℝ) (V : Fin i → ℝ), wuSourceBox k δ N i Δ V →
      ∀ p : SecondFunctionalParameters, p.MotherAdmissible →
      (∑ j : Fin 6, (sourceFamily N δ Δ V p j).noncoprimePart.primeMass) ≤
        ε * boxTheta N ((N : ℝ)^(1/2-δ)) (convolutionWuWindows N Δ V) := by
  let C := 6 * (max 1 (1/(wuLocalExponent k δ / 10)))^(k+3) / log 2
  have hlog2 : 0 < log (2 : ℝ) := log_pos (by norm_num)
  have hC : 0 < C := by dsimp [C]; positivity
  obtain ⟨T1,hT1,hm⟩ := source_bad_primeMass_six_log_uniform k hδ hδhi
  obtain ⟨T2,_,ha⟩ := omega3_absolute_power_log_relative k 1 hδ hδhi hε hC
    (show (0 : ℝ) < 1 by norm_num)
  refine ⟨max T1 T2,hT1.trans (le_max_left _ _),?_⟩
  intro N hN i Δ V hb p hp
...
/-- Genuine consumption of both all-six exception payments, with one total epsilon. -/
theorem source_primeMass_six_le_rough_payment (k : ℕ) {δ ε : ℝ}
    (hδ : 0 < δ) (hδhi : δ < 1/2) (hε : 0 < ε) :
    ∃ T : ℕ, 4 ≤ T ∧ ∀ N : ℕ, T ≤ N →
      ∀ (i : ℕ) (Δ : ℝ) (V : Fin i → ℝ), wuSourceBox k δ N i Δ V →
      ∀ p : SecondFunctionalParameters, p.MotherAdmissible →
      (∑ j : Fin 6, (sourceFamily N δ Δ V p j).primeMass) ≤
        ...
        ε * boxTheta N ((N : ℝ)^(1/2-δ)) (convolutionWuWindows N Δ V) := by
  obtain ⟨T0,hT0,h0⟩ := source_prime_six_le_rough_square_bad k hδ hδhi
  obtain ⟨T1,_,h1⟩ := source_squareRawMass_six_payment k hδ hδhi
    (show 0 < ε/2 by positivity)
  obtain ⟨T2,_,h2⟩ := source_bad_primeMass_six_payment k hδ hδhi
    (show 0 < ε/2 by positivity)
  refine ⟨max T0 (max T1 T2),hT0.trans (le_max_left _ _),?_⟩
  intro N hN i Δ V hb p hp
  have hN0 := (le_max_left T0 (max T1 T2)).trans hN
...
theorem source_prime_purification (k : ℕ) {δ ε : ℝ}
  ...
  obtain ⟨T2,_,h2⟩ := source_primeMass_six_le_rough_payment k hδ hδhi hε
  refine ⟨max T1 T2,hT1.trans (le_max_left _ _),?_⟩
  intro N hN i Δ V hb p hp
