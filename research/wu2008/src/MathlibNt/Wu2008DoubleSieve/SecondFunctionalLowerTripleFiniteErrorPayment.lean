...
def tupleList (t : PrimeTriple) : List ℕ := [t.1,t.2.1,t.2.2]

theorem tupleList_length (t : PrimeTriple) : (tupleList t).length = 3 := rfl

theorem tupleProduct_literal (d : ℕ) (t : PrimeTriple) :
    tupleProduct d t = d * (tupleList t).prod := by
  simp only [tupleProduct, tupleList, List.prod_cons, List.prod_nil]
  ring

def tupleCoordinates (t : PrimeTriple) : Fin 3 → ℕ :=
  fun j => (tupleList t).getD j 0

theorem tupleCoordinates_injective {t u : PrimeTriple}
...
theorem tupleCoordinates_ofFn (t : PrimeTriple) :
...
theorem tupleCoordinates_product (t : PrimeTriple) :
...
theorem actual_tuple_cube {k i N d : ℕ} {δ Δ : ℝ} {V : Fin i → ℝ}
  ...
  have huppers : ∀ j' : Fin 6,
    ...
    unfold LowerTripleGrouped.actualBands LowerTripleGrouped.bands
    refine Fin.cases ?_ (Fin.cases ?_ (Fin.cases ?_ (Fin.cases ?_ (Fin.cases ?_
      (Fin.cases ?_ (fun i => Fin.elim0 i))))))
    · exact hhi
    · exact hk3
    · exact hhi
    · exact hhi
  ...
  · exact (mem_primesIcc (rpow_nonneg (le_of_lt (lt_trans zero_lt_one hR)) _)).mpr
      ⟨hr',hlo.trans ((mem_primeWindow.mp hq').2.2.1.trans
        ((le_max_left _ _).trans hl.le)),hu.trans hupper⟩

/-- Complete ordered labels embed; the residual n is never a coordinate. -/
noncomputable def tupleCubeEmbedding {R : ℝ} (S : Finset PrimeTriple)
    (hmem : ∀ t ∈ S, ∀ r ∈ tupleList t, r ∈ primeSlabPrimes R) :
    S ↪ (Fin 3 → primeSlabPrimes R) where
...
theorem tupleCubeEmbedding_weight {R : ℝ} (S : Finset PrimeTriple)
...
theorem tuple_reciprocal_le {R : ℝ} (S : Finset PrimeTriple)
    (hmem : ∀ t ∈ S, ∀ r ∈ tupleList t, r ∈ primeSlabPrimes R)
    (hmass : (∑ q ∈ primeSlabPrimes R, 1/(q:ℝ)) ≤ 5) :
    (∑ t ∈ S, 1/((tupleList t).prod : ℝ)) ≤ 5^3 := by
  ...
  have he : (∑ t ∈ univ.image E, primeSlabWeight R t) =
    ...
    exact sum_coe_sort S (fun t : PrimeTriple => 1/((tupleList t).prod : ℝ))
  rw [he] at hsub
  exact hsub.trans (by
    rw [show (∑ t : Fin 3 → primeSlabPrimes R, primeSlabWeight R t) =
      (∑ q ∈ primeSlabPrimes R, 1/(q:ℝ))^3 from by
        simpa only [primeSlabWeight, Fintype.card_fin] using primeSlab_product_mass (α := Fin 3) R]
    exact pow_le_pow_left₀ (sum_nonneg (fun _ _ => by positivity)) hmass _)

/-- Pointwise original-weight payment; all three coordinates carry reciprocal weight. -/
...
/-- A common actual slab threshold pays each band and their full sum. -/
theorem finiteErrorMass_six_payment (k : ℕ) {δ : ℝ}
    (hδ : 0 < δ) (hδhi : δ < 1/2) :
    ∃ T : ℕ, 4 ≤ T ∧ ∀ N : ℕ, T ≤ N →
      ∀ (i : ℕ) (Δ : ℝ) (V : Fin i → ℝ), wuSourceBox k δ N i Δ V →
      ∀ p : SecondFunctionalParameters, p.MotherAdmissible → 2 ≤ p.s →
      (∀ j : Fin 6, finiteErrorMass N δ Δ V p j ≤
        5^3/(wuLocalExponent k δ/10) * ((N:ℝ)/log N) *
          boxConvolutionReciprocalMass (convolutionWuWindows N Δ V)) ∧
      (∑ j : Fin 6, finiteErrorMass N δ Δ V p j) ≤
        (6*5^3)/(wuLocalExponent k δ/10) * ((N:ℝ)/log N) *
...
one epsilon, not six epsilon; no division by the convolution mass occurs. -/
theorem rough_mass_six_finite_paid (k : ℕ) {δ ε : ℝ}
  ...
  let C : ℝ := (6*5^3)/(wuLocalExponent k δ/10)
  ...
  obtain ⟨T1,_,h1⟩ := rough_mass_six_finite_buchstab k hδ hδhi (div_pos hε hC)
  obtain ⟨T2,_,h2⟩ := finiteErrorMass_six_payment k hδ hδhi
  ...
  have hsingle : ∀ j : Fin 6, finiteErrorMass N δ Δ V p j ≤ C * X := by
    intro j
    refine (he.1 j).trans ?_
    change (5^3/(wuLocalExponent k δ/10))*((N:ℝ)/log N)*_ ≤ _
    rw [mul_assoc]
