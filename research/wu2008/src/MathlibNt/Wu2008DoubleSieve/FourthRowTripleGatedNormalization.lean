import MathlibNt.Wu2008DoubleSieve.FourthRowTripleGatedMain
import MathlibNt.Wu2008DoubleSieve.Omega3XIntegralNormalization

/-! # Sharp normalization of the gated masses on the unchanged boxTheta -/
...
theorem fourthRowTripleGated_integral_scaled {i k N : ℕ} {δ Δ K τ : ℝ}
    {V : Fin i → ℝ} (hN : 4 ≤ N) (hδ : 0 < δ) (hδhi : δ < 1 / 2)
    (hb : wuSourceBox k δ N i Δ V) (hK : 0 ≤ K)
    (hli : (N : ℝ) / log N ≤ (1 + τ) * logarithmicIntegral N) (ten : Bool) :
    let W := convolutionWuWindows N Δ V
    fourthRowTripleGatedIntegralMain N δ W ten * (K * wuSingularSeries N / log N) ≤
      ((1 + τ) * K / 4 * fourthRowTripleGatedEnvelope ten) *
        boxTheta N ((N : ℝ) ^ (1 / 2 - δ)) W := by
  let W := convolutionWuWindows N Δ V
  let Q := (N : ℝ) ^ (1 / 2 - δ)
  let E := fourthRowTripleGatedEnvelope ten
  let A := ∑ d ∈ boxConvolutionSupport W, (convolutionCoeff W d : ℝ) / (d * log (Q / d))
  ...
  have hE : 0 ≤ E := (fourthRowTripleGated_envelope_bounds ten).1
  ...
  have hS : wuSingularSeries N * A ≤ U := by
    ...
    apply sum_le_sum
    intro d hd
    have hd0 := boxConvolutionSupport_pos (fun j p hp => (mem_convolutionWuWindows.mp hp).1.pos) hd
  ...
  have he : fourthRowTripleGatedIntegralMain N δ W ten ≤ (N : ℝ) * E * A := by
    unfold fourthRowTripleGatedIntegralMain
    rw [mul_assoc]
    ...
    have hφ : 2 ≤ omega3XPhi N d δ := by
      ...
      linarith [hg.2.2.1]
    simpa only [mul_comm] using mul_le_mul_of_nonneg_left (fourthRowTripleGated_le_envelope ten hφ)
...
theorem fourthRowTripleGated_X_scaled (k : ℕ) {δ K τ ε : ℝ}
    ...
    ∃ T : ℕ, 4 ≤ T ∧ ∀ N : ℕ, T ≤ N →
      ...
      fourthRowTripleGatedX N δ W (fourthRowTripleGatedProfiles N δ W ten) *
        ...
        (((1 + τ) * K / 4 * fourthRowTripleGatedEnvelope ten) + ε * K / 2) * boxTheta N ((N : ℝ) ^ (1 / 2 - δ)) W := by
  obtain ⟨TM, hTM4, hTM⟩ := fourthRowTripleGated_X_main k hδ hδhi hε
  ...
  have hm := hTM N ((le_max_left _ _).trans hN) i Δ V hb ten
  have hn := fourthRowTripleGated_integral_scaled hN4 hδ hδhi hb hK
    (hTL N ((le_max_right _ _).trans hN)) ten
  have he := omega3X_scaled_error_le hN4 hδ hδhi hb hε.le hK
  dsimp only at hm hn he ⊢
  have hmul := mul_le_mul_of_nonneg_right hm (show 0 ≤ K * wuSingularSeries N / log N by positivity)
