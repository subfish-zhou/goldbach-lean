...
/-- Six original families pay R2 and closed Small from one epsilon budget.
...
theorem source_R2_small_payment (k : ℕ) {δ ε : ℝ}
    ...
    ∃ T : ℕ, 4 ≤ T ∧ ∀ N : ℕ, T ≤ N →
      ...
      let E := ∑ j : Fin 6, (L j).R2 D Z
      let S := ∑ j : Fin 6, LowerTripleGroupedOutput.closedSmall N δ Δ V p j Z
      1 < D ∧ Z ≤ (D : ℝ) ∧ Z ≤ N ∧
      (∀ q ∈ omega3SieveModuli N D Z, q ≤ N) ∧
      E ≤ (ε/2)*boxTheta N Q W ∧ S ≤ (ε/2)*boxTheta N Q W ∧
      E + S ≤ ε*boxTheta N Q W ∧
      E + (∑ j : Fin 6, (L j).small Z) ≤ ε*boxTheta N Q W ∧
      ∀ P : Fin 6 → Label → Prop,
        let LP := fun j => (L j).restrictLabels (P j)
        (∑ j : Fin 6, (LP j).R2 D Z) ≤ (ε/2)*boxTheta N Q W ∧
        (∑ j : Fin 6, (LP j).small Z) ≤ (ε/2)*boxTheta N Q W ∧
        (∑ j : Fin 6, (LP j).R2 D Z) + (∑ j : Fin 6, (LP j).small Z) ≤
          ε*boxTheta N Q W := by
  ...
  have hmod : ∀ q ∈ omega3SieveModuli N D Z, q ≤ N := by
    ...
    omega
  have hrj (j : Fin 6) := hEuler N (by omega) _ (L j) D Z ((N : ℝ)^η) F
    hgeom.2.2.2.1 (rpow_pos_of_pos hNr _) hF.le hmod
    (hin.1 j).1 (hin.1 j).2
  have hr : (∑ j : Fin 6, (L j).R2 D Z) ≤ (ε/2)*boxTheta N Q W := by
    calc
      _ ≤ ∑ _j : Fin 6, C*F*N*((1+log N)*log N^4/((N : ℝ)^η*log 2)) :=
        sum_le_sum (fun j _ => hrj j)
      _ = 6*C*F*N*((1+log N)*log N^4/((N : ℝ)^η*log 2)) := by simp; ring
      _ ≤ 6*C*F*N*((2*log N)*log N^4/((N : ℝ)^η*log 2)) := by
        gcongr
        linarith
      _ = (12*C*F/log 2)*N*log N^5/(N : ℝ)^η := by ring
      _ ≤ _ := hT1 N hN1 i Δ V hb
  have hs : (∑ j : Fin 6, LowerTripleGroupedOutput.closedSmall N δ Δ V p j Z) ≤
      (ε/2)*boxTheta N Q W := by
    calc
      _ ≤ 6*G*Z := hin.2 Z hpow.2.2.2.1
      _ = (6*G)*N*log N^(0 : ℕ)/(N : ℝ)^(1-β) := hpow.2.2.2.2.2 (6*G)
      _ ≤ _ := hT2 N hN2' i Δ V hb
  have hsc : (∑ j : Fin 6, (L j).small Z) ≤
      ∑ j : Fin 6, LowerTripleGroupedOutput.closedSmall N δ Δ V p j Z :=
    sum_le_sum (fun j _ => LowerTripleGroupedOutput.source_small_le_closed N δ Δ V p j Z)
  refine ⟨hgeom.2.2.2.2.1, hgeom.2.2.2.2.2.1, hgeom.2.2.2.1, hmod,
    hr, hs, by linarith, by linarith, ?_⟩
  intro P
  have hrP : (∑ j : Fin 6, ((L j).restrictLabels (P j)).R2 D Z) ≤
      ∑ j : Fin 6, (L j).R2 D Z :=
    sum_le_sum (fun j _ => (L j).restrictLabels_R2_le (P j) D Z)
  have hsP : (∑ j : Fin 6, ((L j).restrictLabels (P j)).small Z) ≤
      ∑ j : Fin 6, (L j).small Z :=
    sum_le_sum (fun j _ => (L j).restrictLabels_small_le (P j) Z)
  exact ⟨hrP.trans hr, (hsP.trans hsc).trans hs, by dsimp only; linarith⟩

/-- A consumer-facing arbitrary six-label restriction at the same source scales. -/
theorem source_restricted_R2_small_payment (k : ℕ) {δ ε : ℝ}
    ...
    ∃ T : ℕ, 4 ≤ T ∧ ∀ N : ℕ, T ≤ N →
      ...
      1 < D ∧ Z ≤ (D : ℝ) ∧ Z ≤ N ∧
      (∀ q ∈ omega3SieveModuli N D Z, q ≤ N) ∧
      (∑ j : Fin 6, (L j).R2 D Z) ≤ (ε/2)*boxTheta N Q W ∧
      (∑ j : Fin 6, (L j).small Z) ≤ (ε/2)*boxTheta N Q W ∧
      (∑ j : Fin 6, (L j).R2 D Z) + (∑ j : Fin 6, (L j).small Z) ≤
        ε*boxTheta N Q W := by
