import MathlibNt.Wu2008DoubleSieve.GCurvatureIntegral
import MathlibNt.Wu2008DoubleSieve.SixthLogCubicCount
...
noncomputable def newBalance : ℝ :=
  BaseRecurrenceLower.newBase+UnroundedPayments.fifthRational+
    RationalMovingSixth.rationalSixth+SixthReciprocalCorrection.deltaSixth+SixthLogCubicCorrection.deltaLog+47/481250-(GConvexChord.rationalG-deltaG)-
...
theorem complete_coefficient_gt_newBalance :
  ...
  change SingleUpperClassicalLimit.Glin (1/3)+
    SingleUpperClassicalLimit.Glin truncatedSixthLowerSigma ≤ GConvexChord.rationalG-deltaG at hg
...
theorem exact_balance_gain : newBalance = SixthLogCubicCorrection.newBalance+deltaG := by
  unfold newBalance SixthLogCubicCorrection.newBalance
  ring

theorem balance_strictly_improved : SixthLogCubicCorrection.newBalance < newBalance := by
  rw [exact_balance_gain]
  linarith only [deltaG_pos]

noncomputable def exactBalance : ℝ := SixthLogCubicCorrection.exactBalance+400/35829

theorem newBalance_eq_exact : newBalance = exactBalance := by
  rw [exact_balance_gain,SixthLogCubicCorrection.newBalance_eq_exact,deltaG_exact]
  rfl
...
theorem exact_quarter_gain :
    exactQuarter = SixthLogCubicCorrection.exactQuarter+100/35829 := by
  unfold exactQuarter exactBalance SixthLogCubicCorrection.exactQuarter
  ring
