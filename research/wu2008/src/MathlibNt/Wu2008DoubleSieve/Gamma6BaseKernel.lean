...
theorem gamma6Base_section_regular {C D : ℝ}
  ...
  constructor
  · intro t
    have h := primeOrdered_integral_norm_le_four hc hd (M := 4) (by norm_num)
  ...
  · intro x y
    ...
    change |gamma6BaseSection C D x - gamma6BaseSection C D y| ≤ 4 * (16 * |x - y|) at h
    nlinarith
...
theorem gamma6Base_integral_bounds {A B C D : ℝ}
  ...
  · have h := primeOrdered_integral_norm_le_four ⟨hAa, hAB.trans hBb⟩ ⟨hAa.trans hAB, hBb⟩
      (M := 16) (by norm_num) (fun t _ => hr.1 t)
    linarith [(le_abs_self (∫ t in A..B, gamma6BaseSection C D t / t))]

theorem gamma6Base_inner_integrable {t C D : ℝ}
    (_ht : gamma5MassA ≤ t) (htb : t ≤ gamma6BaseB)
    (hC : gamma6BaseC ≤ C) (hCD : C ≤ D) (hD : D ≤ gamma6BaseF) :
