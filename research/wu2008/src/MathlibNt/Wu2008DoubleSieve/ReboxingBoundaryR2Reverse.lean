...
/-!
# Full reverse R2 payment

The actual nonnegative difference `S(cutoff(s2))-S(p)` is summed over
all geometric blocks before payment. The half-open partition removes
the moving block count, with every convolution multiplicity unchanged.
The resulting relative rate is `O(log(N)^(-2))`, sufficient for epsilon
removal; no sharper version of Wu04 (3.19) is asserted.
-/
...
noncomputable def reboxingR2Reverse {i : ℕ} (N : ℕ) (δ Δ : ℝ) (V : Fin i → ℝ)
    (t : ℝ) (r : ℕ) : ℝ :=
...
/-- A single threshold, before every source box and geometric terminal
index, pays the entire reverse error against that same box's Theta.
Depth zero, `s=2`, `s=t`, and the zero-block sum are included. -/
theorem reboxingR2Reverse_relative (k : ℕ) {δ ε : ℝ}
    ...
    ∃ T : ℕ, ∀ N : ℕ, T ≤ N → Even N →
      ...
      reboxingAlpha ((N : ℝ) ^ (1 / 2 - δ) / (∏ l, V l)) Δ t r ≤
        ((N : ℝ) ^ (1 / 2 - δ) / (∏ l, V l)) ^ (1 / s) →
      0 ≤ reboxingR2Reverse N δ Δ V t r ∧ reboxingR2Reverse N δ Δ V t r ≤
        ε * boxTheta N ((N : ℝ) ^ (1 / 2 - δ)) (convolutionWuWindows N Δ V) := by
  obtain ⟨B, hB, T1, hT1⟩ := reboxingR2Reverse_atom_bound k hδ hδhi
  ...
  have hnon : 0 ≤ reboxingR2Reverse N δ Δ V t r := by
    ...
    exact (hh hp).1
  have haggregate : reboxingR2Reverse N δ Δ V t r ≤
