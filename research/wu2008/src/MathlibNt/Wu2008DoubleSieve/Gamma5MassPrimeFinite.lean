...
theorem gamma5Mass_pairs_eq_rows {N : ℕ} {R A B C D : ℝ}
  ...
  · rw [if_pos hpN]
    ...
    have he : gamma5MassRowPrimes N R C D p =
      ...
      exact gamma5Mass_row_mem_iff hR hCD hpp
    rw [he, sum_filter]
    apply sum_congr rfl
    intro q _
    by_cases hq : p < q ∧ (q : ℝ) < R ^ gamma5ClassicalB ∧ q.Coprime N
    · rw [if_pos ⟨hq.1, hq.2.1, hpN, hq.2.2⟩, if_pos hq]
      ring
...
theorem gamma5Mass_row_boundary {p : ℕ} {R C D Z M : ℝ}
  ...
  have hcard : E.card ≤ 3 :=
    (Finset.card_le_card hsub).trans (by
      calc
        _ ≤ ({⌊R ^ gamma5ClassicalB⌋₊, ⌊R ^ D⌋₊} : Finset ℕ).card + 1 := card_insert_le _ _
