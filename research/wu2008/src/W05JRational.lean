...
theorem rationalKernel_le {S A b u : ℝ} (hA : 2 ≤ A) (hAS : A ≤ S - 1)
  ...
  have hj : (u - (A - 1)) * (1 / (b * (b + 1)) + 1 / (b * (S - A))) ≤
    ...
    calc
      _ = (u - (A - 1)) / (b * (b + 1)) +
          (u - (A - 1)) / (b * (S - A)) := by ring
      _ ≤ _ := by simpa only [add_div] using (add_le_add h3 h4).trans h5
...
theorem rationalJCoefficients_paid {z : Fin 9 → ℝ} (hz : ∀ k, 0 ≤ z k) {s S : ℝ}
  ...
  exact add_le_add le_rfl (Finset.sum_le_sum (fun k _ =>
    mul_le_mul_of_nonneg_right (rationalJCoefficient_le hs hS hS5 hsS hr k) (hz k)))
