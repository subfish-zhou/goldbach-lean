...
/-- The actual three-coordinate Theta pays the one four-label BV error.
Mass uses exactly i=k=3, hence log^(15+2), leaving one log from BV saving 18. -/
theorem inserted_AP_relative {δ ε : ℝ}
    ...
    ∃ T : ℕ, 4 ≤ T ∧ ∀ N : ℕ, T ≤ N → ∀ Δ : ℝ,
      ...
      ε*boxTheta N ((N : ℝ)^(1/2-δ)) (convolutionWuWindows N Δ (Fin.cons U V)) := by
  have hη : 0 < highEta := by norm_num [highEta]
  obtain ⟨C,hC,T0,hT04,hAP⟩ := inserted_AP_paid hδ hδhi
  ...
  have htheta : c*N/log N^17 ≤
      boxTheta N ((N : ℝ)^(1/2-δ)) (convolutionWuWindows N Δ (Fin.cons U V)) := by
    have hm := (hmass N hN1 3 le_rfl Δ hlo hhi (Fin.cons U V) hend.1 hend.2).1
  ...
  have hb : C/log N ≤ ε*c := by
    ...
    nlinarith
  refine ⟨(by positivity : 0 ≤ c*N/log N^17).trans htheta,?_⟩
  calc
    _ ≤ C*N/log (N : ℝ)^(18 : ℝ) := hAP N hN0 Δ hlo hhi V hV hr U hU
