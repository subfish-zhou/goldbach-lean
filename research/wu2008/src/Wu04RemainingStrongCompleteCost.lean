import Wu04RemainingStrongFourthMass

namespace Wu04RemainingStrongCompleteCost
open Wu2008DoubleSieve ActualNineFeedback NodeExtension Wu08OriginalPsiRecovery
open Wu04RemainingCore SecondFunctionalFourSevenths SecondFunctionalJointTail SecondFunctionalGeometricMass
open scoped BigOperators
noncomputable section

def costCap (i : Fin 3) : ℝ := Wu04RemainingStrongPrefixCost.costCap (row i)-
  Wu04RemainingStrongGammaMass.gain i-Wu04RemainingStrongFourthMass.gain i
...
theorem six_paid (i : Fin 3) {φ : ℝ} (hφ : 2≤φ) :
    ...
    Wu04MainTail.cap*(∑ j : Fin 6,lowerMass (row i) j)-
  ...
  have hp (j : Fin 6) : LowerTripleContinuous.K (1/(row i).S) (1/(row i).kappa1)
      ...
      (if j=3 then Wu04RemainingStrongFourthMass.gain i else 0) := by
    by_cases h2 : j=2
    · subst j
      simpa only [ite_true,show (2:Fin 6)≠3 by decide,ite_false,sub_zero] using
        Wu04RemainingStrongThird.kernel_paid i hφ
    · by_cases h3 : j=3
      · subst j
        simpa only [h2,ite_false,ite_true,sub_zero] using Wu04RemainingStrongFourthMass.kernel_paid i hφ
      · simpa only [h2,h3,ite_false,sub_zero] using Wu04RemainingLower.lower_paid i j hφ
  have hs := Finset.sum_le_sum (fun j (_ : j∈(Finset.univ : Finset (Fin 6))) => hp j)
  simpa only [Finset.sum_sub_distrib,← Finset.mul_sum,Finset.sum_ite_eq',Finset.mem_univ,ite_true] using hs

theorem joint_paid (i : Fin 3) {φ : ℝ} (hφ : 2≤φ) :
    SecondFunctionalCoupled.kernel (row i) φ≤Wu04RemainingCost.jointCore (row i)-
      Wu04RemainingStrongThird.gain (row i)-Wu04RemainingStrongFourthMass.gain i := by
  ...
  unfold SecondFunctionalCoupled.kernel Wu04RemainingCost.jointCore Wu04HighCoupledSupport.highKernel at *
  linarith only [hl,hf,hh]

/-- Each supremum is bounded separately; all unit/legal and fourprime terms remain. -/
theorem complete_paid (i : Fin 3) : coupledCostMass (row i)≤costCap i := by
  have hc := original_cost_of_pointwise (fun _ hφ => Wu04RemainingStrongGammaMass.remaining_omega i hφ)
    (fun _ hφ => joint_paid i hφ)
  have he := Wu04RemainingCost.complete_identity (row i) (original_mother i.succ) (original_s_ge_two i.succ)
  ...
  unfold costCap Wu04RemainingStrongPrefixCost.costCap
  linarith only [hc,he,hb]

theorem strictly_better (i : Fin 3) : costCap i<Wu04RemainingStrongPrefixCost.costCap (row i) := by
  unfold costCap
  linarith only [Wu04RemainingStrongGammaMass.gain_pos i,Wu04RemainingStrongFourthMass.gain_pos i]

/-- New actual consumer includes Gamma9 and the clipped original fourth-domain gain. -/
theorem actual_remaining (i : Fin 3) {δ : ℝ} (hd : 0<δ) (hh : δ≤1/10) :
    Wu04RemainingStrongClassical.certificate (row i)/5-2*costCap i/(5*(1-2*δ))+
...
end Wu04RemainingStrongCompleteCost
