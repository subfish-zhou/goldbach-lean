...
theorem Gamma_expansion (w : Fin 21 → ℝ) {t : ℝ} (ht : 0 ≤ t) :
  ...
  rw [integral_finsetSum _ (fun j _ => (uniform_integrable (basis j) ht).const_mul (w j))]
...
theorem eval_basis (L : List (Fin 21)) (j : Fin 21) (s : ℝ) :
  ...
  | cons k L ih =>
    ...
    by_cases hk : rNode k.val < s ∧ s ≤ rNode (k.val + 1)
    · rw [if_pos hk]
      by_cases hkj : k = j
      · subst k
        simp [basis, cell, hk]
...
theorem profile_basis (j : Fin 21) (s : ℝ) :
  ...
  simpa only [profile, rows, List.mem_finRange, true_and] using
    eval_basis (List.finRange 21) j s
