...
theorem triangle_low_ratio {c b : ℝ} (hc : c ≠ 0) (hb : c - b ≠ 0) :
    (c - (c - b)) * (c / 2) / ((c - b) * (c - c / 2)) = b / (c - b) := by
  have hh : c - c / 2 ≠ 0 := by intro h; apply hc; linarith
  field_simp
  ring

theorem triangle_high_ratio {c a : ℝ} (hc : c ≠ 0) (ha : a ≠ 0) :
    (c - a) * (c / 2) / (a * (c - c / 2)) = (c - a) / a := by
  have hh : c - c / 2 ≠ 0 := by intro h; apply hc; linarith
  field_simp
  ring
...
theorem source66_section {p : SecondFunctionalParameters} (hp : PropositionFourGeometry p)
  ...
  · have hc1 : 1 / p.S + 1 / p.kappa2 ≤ 1 - v / p.S := by
    ...
    have hratio :
      ...
      rw [triangle_low_ratio hc0.ne' hcne]
      field_simp
      ring
    rw [hratio]
    ring
  · by_cases hhigh : v ∈ Icc (alpha4 p) (alpha2 p)
    · have hc1 : 2 * (1 / p.S) ≤ 1 - v / p.S := by
      ...
      have hratio :
        ...
        rw [triangle_high_ratio hcne ha0.ne']
        field_simp
        ring
      rw [hratio]
      ring
    · have hnot : v < alpha6 p ∨ alpha2 p < v := by
        ...
        · exfalso; linarith
        · exact Or.inr h'
      rw [strip_section_empty (l := fun x => p.S * (1 - 1 / p.kappa2 - x))
        (u := fun x => p.S * (1 - 2 * x)) (v := v) hab (by
        intro x hx hz
        ...
        simp only [div_eq_mul_inv] at hxlo hxhi hz hnot
