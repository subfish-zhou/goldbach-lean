...
theorem g16_strict : Phase15.g15 < g16 := by
  unfold Phase15.g15 g16
  apply (div_lt_div_iff_of_pos_right (sub_pos.mpr rhoEndpoint_bounds.2)).2
  exact mul_lt_mul_of_pos_right C16_strict (by norm_num [sourceP])

/-- The new unconditional raw-integral producer is actually supplied here. -/
theorem complete_reciprocal_actual_count :
    ∃ T : ℕ, 512 ≤ T ∧ ∀ N : ℕ, T ≤ N → Even N →
      (GCurvatureChord.exactQuarter+
        firstFunctionalGainPsiOne HighSix.s HighSix.S*HighSix.primeIntegral 0+g16)*
        wuSingularSeries N*N/log N^(2 : ℕ) ≤
          ((MathlibNt.Wu2008DoubleSieve.wuPrimeComplements N).card : ℝ) :=
  Phase10.same_mother_exact_payment g16 B16 B16_pos
    (fun _ hδ hδhi => gain16_linear_error hδ hδhi)
...
theorem complete_reciprocal_ordinary_P2_same_threshold :
    ∃ T : ℕ, 512 ≤ T ∧ ∀ N : ℕ, T ≤ N → Even N →
      ((GCurvatureChord.exactQuarter+
          (3629895479136046171/11308652260919376406875 : ℝ)+g16)*
        wuSingularSeries N*N/log N^(2 : ℕ) ≤
          ((MathlibNt.Wu2008DoubleSieve.wuPrimeComplements N).card : ℝ)) ∧
      ((GCurvatureChord.exactQuarter+
          (3629895479136046171/11308652260919376406875 : ℝ)+g16)*
  ...
  obtain ⟨T,hT,h⟩ := complete_reciprocal_actual_count
  refine ⟨T,hT,?_⟩
  intro N hN he
  ...
  have hc : GCurvatureChord.exactQuarter+
      (3629895479136046171/11308652260919376406875 : ℝ)+g16 ≤
      GCurvatureChord.exactQuarter+
      firstFunctionalGainPsiOne HighSix.s HighSix.S*HighSix.primeIntegral 0+g16 := by
