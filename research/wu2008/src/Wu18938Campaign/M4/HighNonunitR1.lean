...
theorem original_nonunit_subfamily_R1_log_saving {δ A : ℝ}
    (hd : 0 < δ) (hh : δ ≤ 1 / 100) (hA : 0 < A) :
    ∃ C : ℝ, 0 < C ∧ ∃ T : ℕ, 4 ≤ T ∧ ∀ N : ℕ, T ≤ N →
      ∀ j : Fin 3, ∀ high : Bool, ∀ G : LabelledPhysical.Family Profile N,
      G.labels ⊆ (originalNonunitFamily j N δ high).labels →
      (∀ x, G.weight x = (convolutionCoeff (windows j N) x.1 : ℝ) ∧
  ...
  refine ⟨C, hC, T, hT, ?_⟩
  intro N hN j high G hsub hdata Z
...
theorem original_nonunit_R1_log_saving {δ A : ℝ}
    ...
    ∃ C : ℝ, 0 < C ∧ ∃ T : ℕ, 4 ≤ T ∧ ∀ N : ℕ, T ≤ N →
      ...
      (if good then L.coprimePart else L).R1 (⌊(N : ℝ) ^ (1 / 2 - δ)⌋₊ + 1) Z ≤
        C * N / log (N : ℝ) ^ A := by
  obtain ⟨C, hC, T, hT, hb⟩ := original_nonunit_subfamily_R1_log_saving hd hh hA
  refine ⟨C, hC, T, hT, ?_⟩
  intro N hN j high good Z
  apply hb N hN j high
  · cases good
    · exact Subset.rfl
...
theorem original_nonunit_subfamily_R1_paid {δ ε : ℝ}
  ...
  obtain ⟨C, _, T0, hT04, hb⟩ := original_nonunit_subfamily_R1_log_saving hd hh
  ...
  have hc : C / log (N : ℝ) ≤ ε * wuSingularSeries N := by
    ...
    nlinarith
  calc
    _ ≤ C * N / log (N : ℝ) ^ (3 : ℝ) := hb N (by omega) j high G hsub hdata Z
...
theorem original_nonunit_R1_paid {δ ε : ℝ}
    ...
    ∃ T : ℕ, 4 ≤ T ∧ ∀ N : ℕ, T ≤ N → ∀ j : Fin 3, ∀ high good : Bool, ∀ Z : ℝ,
      ...
      (if good then L.coprimePart else L).R1 (⌊(N : ℝ) ^ (1 / 2 - δ)⌋₊ + 1) Z ≤
        ε * truncatedSixthMassScale N := by
  obtain ⟨T, hT, hb⟩ := original_nonunit_subfamily_R1_paid hd hh heps
  refine ⟨T, hT, ?_⟩
  intro N hN j high good Z
  apply hb N hN j high
  · cases good
...
theorem original_nonunit_prime_upper_without_R1 {δ ε : ℝ}
  ...
  refine ⟨T, hT, ?_⟩
  intro N hN hEven j high D Z L
  have hg := omega3_source_sieve_geometry (by omega : 2 ≤ N) hd
    (show δ < 1 / 2 by linarith)
  have hp := L.prime_upper_coprime_finite_sum (by omega) hEven D Z
