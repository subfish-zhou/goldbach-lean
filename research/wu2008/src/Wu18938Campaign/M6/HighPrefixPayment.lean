...
theorem high_prefix_paid (A : ℕ) {e ε δ η ρ σ : ℝ}
  ...
  have hmesh := hm N hNm e (fun _ => (N : ℝ) / (Real.log N) ^ (A + 4))
    (by intro k hk; simp only [one_mul]; exact le_rfl)
  ...
  calc
    _ ≤ _ := hp
    _ ≤ (∑ k ∈ U8Literal.occupied N e ρ, OriginalU8.actualCenter N ρ δ η k P z) +
        (σ / 2) * (N : ℝ) / (Real.log N) ^ A +
        (σ / 2) * (N : ℝ) / (Real.log N) ^ A :=
      add_le_add (add_le_add le_rfl hmesh) hout
    _ = _ := by ring
...
theorem paperSmallEighth_normalized_add_low (τ : ℝ) (hτ : 0 < τ) :
    ∃ η : ℝ, 0 < η ∧ η < 1 / 8 ∧ ∀ (A : ℕ) (e ε δ ρ σ : ℝ),
      (3 / 4 : ℝ) ≤ e → e ≤ 1 → 0 < ε → ε < 100 / 1327 →
      ε < δ → δ < 1 / 4 → 1 < ρ → ρ ≤ 5 / 4 → 0 < σ →
      ∃ N₀ : ℝ, ∀ N : ℕ, N₀ ≤ (N : ℝ) → Even N →
      ((paperSmallEighth N).card : ℝ) ≤
        4 * (1 + τ) * SingularSeries.liuSingularSeries N *
          (∑ k ∈ U8Literal.occupied N e ρ,
            OriginalU8.rectangleMass N ρ k / Real.log (OriginalU8.level N ρ δ k)) +
          σ * (N : ℝ) / (Real.log N) ^ A + (U8Literal.smallPrefix N e).card := by
  obtain ⟨η, hη, hηu, hprefix⟩ := high_prefix_normalized τ hτ
  refine ⟨η, hη, hηu, ?_⟩
  intro A e ε δ ρ σ he he1 hε hεa hεδ hδ hρ hρu hσ
  obtain ⟨N₀, hN₀⟩ := hprefix A e ε δ ρ σ he he1 hε hεa hεδ hδ hρ hρu hσ
  refine ⟨N₀, ?_⟩
  intro N hN hEven
