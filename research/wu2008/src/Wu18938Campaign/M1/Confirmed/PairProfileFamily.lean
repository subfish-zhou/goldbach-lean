...
theorem finite_profile_upper (p : SecondFunctionalParameters) (hp : AnalyticParameters p)
    ...
    (hF : (F : Set (GainRectangle p j)).Pairwise (fun r s =>
    ...
    ∃ ρ : ℝ, 0 < ρ ∧ ∃ T : ℕ, 4 ≤ T ∧ ∀ N : ℕ, T ≤ N → Even N →
      ...
      ∀ H : GainRectangle p j → ℝ,
      (∀ r ∈ F, 0 ≤ H r ∧ H r ≤ 1) →
      (∀ r ∈ F, ∀ (k : ℕ) (U : Fin k → ℝ), RoughBox (m + 2) (childEta p η) δ N k Δ U →
      ...
      secondFunctionalMotherGammaSum p N δ (convolutionWuWindows N Δ V) j.index ≤
        (Pair.classicalIntegral p j -
          (∑ r ∈ F, H r * rectIntegral r.A r.B r.C r.D) + ε) *
          boxTheta N ((N : ℝ) ^ (1 / 2 - δ)) (convolutionWuWindows N Δ V) := by
  let C := Pair.classicalIntegral p j
  have hC : 0 ≤ C := (MotherPair.classicalIntegral_bounds hp j).1
  ...
  choose TP hTP4 hTP using (fun r : F => packing_node p hp j r.val m hη hδ)
  choose TM hTM4 hTM using (fun r : F => packing_mass_lower p hp j r.val m hη hδ hζ)
  obtain ⟨TC,hTC4,hTC⟩ := term_mask_upper p hp m hη hδ hρ hρ
  obtain ⟨TT,_,hTT⟩ := term_mass p hp m hη hδ hρ
  obtain ⟨TG,_,hTG⟩ := gain_mesh m hη hδ (show (0 : ℝ) < 1 by norm_num)
  ...
  have hclass := hTC N (by omega) heven i Δ V hb j (L \ univ.biUnion P) sdiff_subset
  have hm0 (r : F) : 0 ≤ gamma5ClassicalMainMass N δ W (P r) := by
    have hh := term_mass_mono hb (by omega) hη hδ p hp j (empty_subset (P r)) (hs r)
    simpa only [gamma5ClassicalMainMass,sum_empty,mul_zero] using hh
  have hupper := gamma5Gain_finite_upper hρ.le
    (term_count_partition p j N δ W L P hd hs)
  ...
  have htotal := hTT N (by omega) i Δ V hb j
  ...
  rw [← gamma_dictionary hb (by omega) hη hδ p hp j,← hG]
