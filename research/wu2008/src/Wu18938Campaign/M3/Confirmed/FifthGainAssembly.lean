import Wu18938Campaign.M3.Confirmed.FifthGain
...
def fifthPaidCoefficient : ℝ :=
  paidOriginalCoefficient+(10959/10000000-1359/1000000)/4

def fifthPaidLedger : ℝ :=
  analyticClassicalLedger+(10959/10000000-1359/1000000)/4

theorem fifth_paid_ledger_exact :
    fifthPaidLedger = (44950107/50000000:ℝ) ∧
      fifthPaidLedger-(899/1000:ℝ) = 107/50000000 ∧
      (899/1000:ℝ) < fifthPaidLedger := by
  rw [fifthPaidLedger,analytic_classical_ledger_exact]
  norm_num
...
include hnodes h34 h6

theorem ordinary_count_with_paid_fifth {ε : ℝ} (he : 0 < ε) :
    ∃ T : ℕ, 4 ≤ T ∧ ∀ N : ℕ, T ≤ N → Even N →
      (fifthPaidCoefficient-ε)*truncatedSixthMassScale N ≤
  ...
  obtain ⟨T5,_,hp⟩ := fifth_actual_count_paid he10
  ...
  unfold fifthPaidCoefficient paidOriginalCoefficient
...
theorem ordinary_899_with_paid_fifth {ε : ℝ} (he : 0 < ε) :
    ∃ T : ℕ, 4 ≤ T ∧ ∀ N : ℕ, T ≤ N → Even N →
      ((899/1000:ℝ)-ε)*truncatedSixthMassScale N ≤
        ((MathlibNt.Wu2008DoubleSieve.wuPrimeComplements N).card : ℝ) := by
  obtain ⟨T,hT,h⟩ := ordinary_count_with_paid_fifth hnodes h34 h6 he
  have hc : (899/1000:ℝ) ≤ fifthPaidCoefficient := by
    have hp := coefficient_lower_from_three_classical_inputs hC1 hC3 hC4
    have hm := fifth_paid_ledger_exact.2.2
    unfold fifthPaidCoefficient fifthPaidLedger at *
    linarith only [hp,hm]
...
theorem refined_1894_with_paid_fifth :
    ∃ T : ℕ, 4 ≤ T ∧ ∀ N : ℕ, T ≤ N → Even N →
  ...
  refined_1894_from_ordinary_count
    (fun _ he => ordinary_899_with_paid_fifth hnodes h34 h6 hC1 hC3 hC4 he)
