...
theorem first_weighted_count {ε : ℝ} (hε : 0 < ε) :
  ...
  calc
    (3 * firstLower - ε) * wuSingularSeries N * N / log N ^ (2 : ℕ) =
        3 * ((firstLower - ε / 3) * wuSingularSeries N * N / log N ^ (2 : ℕ)) := by ring
    _ ≤ 3 * (sieveCount N 1 N ((N : ℝ) ^ truncatedSixthLowerAlpha) : ℝ) :=
      mul_le_mul_of_nonneg_left (hcount N hN hEven) (by norm_num)
