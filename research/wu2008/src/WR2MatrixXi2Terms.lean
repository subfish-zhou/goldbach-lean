...
def xi2Left (p : SecondFunctionalParameters) (j : Fin 9) : ℝ :=
  match j.val with
  | 0 => alpha2 p
  | 1 => alpha9 p
  | 2 => alpha5 p
...
def xi2Right (p : SecondFunctionalParameters) (j : Fin 9) : ℝ :=
  match j.val with
  | 0 => 3
  | 1 => alpha1 p
  | 2 => alpha2 p
...
def xi2Numerator (p : SecondFunctionalParameters) (t : ℝ) (j : Fin 9) : ℝ :=
  match j.val with
  | 0 => (t + 1)^5
  | 1 => t + 1
  | 2 => t + 1
...
def xi2Denominator (p : SecondFunctionalParameters) (t : ℝ) (j : Fin 9) : ℝ :=
  match j.val with
  | 0 => xi2Product p
  | 1 => (p.kappa2 - 1) * (p.kappa1 - 1 - t)
  | 2 => (p.kappa3 - 1) * (p.S - 1 - t)
  | 3 => (p.s - 1) * (p.S - 1 - t)
  | 4 => (p.kappa1 - 1) * (p.kappa2 - 1)
  | 5 => (p.kappa1 * p.S - p.S - p.kappa1 * t) *
      (p.kappa3 * p.S - p.S - p.kappa3 * t)
  | 6 => p.kappa1 * p.S - p.S - p.kappa1 * t
  | 7 => p.kappa2 * p.S - p.S - p.kappa2 * t
  | _ => 1
...
theorem xi2_denominator_lower {p : SecondFunctionalParameters} {j : Fin 9} {t : ℝ}
    (hs : 0 ≤ p.s - 1) (hk1 : 0 ≤ p.kappa1)
    (hk2 : 0 ≤ p.kappa2 - 1) (hk3 : 0 ≤ p.kappa3 - 1)
    (hp : 0 < p.kappa1 * p.S - p.S - p.kappa1 * xi2Right p 5 ∧
      0 < p.kappa3 * p.S - p.S - p.kappa3 * xi2Right p 5)
    (hb : t ≤ xi2Right p j) :
    xi2Denominator p (xi2Right p j) j ≤ xi2Denominator p t j := by
  have hk2' : 0 ≤ p.kappa2 := by linarith only [hk2]
  have hk3' : 0 ≤ p.kappa3 := by linarith only [hk3]
  fin_cases j
  · exact le_rfl
...
theorem xi2_denominator_positive (i : Fin 4) (j : Fin 9) {t : ℝ}
  ...
  have hk1 := (coupledRow_geometry i).2.1
  exact (xi2_right_denominator_positive i j).trans_le
    (xi2_denominator_lower (by linarith only [hg.1]) (by linarith only [hk1])
      (by linarith only [hg.2.2.1]) (by linarith only [hg.2.2.2.2.1])
      (xi2_product_factors_positive i) ht.2)
