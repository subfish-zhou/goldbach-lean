...
open Phase17 (κX κY HCoupled rhoCoupled rhoCoupled_bounds amplitudeClosedCoupled)
...
theorem actual_reciprocal_lower {δ t : ℝ} (hδ : 0 < δ) (hδhi : δ ≤ 1/100)
    (ht : t ∈ Icc L0 (movingRight δ)) : amplitude δ*f coupled t ≤ kernel δ t := by
  ...
  have hH := HCoupled hδ hδhi (shape_argument hδ hδhi hts)
  have he : amplitude δ*(κY+(25/468)*(23/5-argument δ t)^2+
      κX*(5/18)*(23/5-argument δ t)) = amplitude δ*poly coupled (t-shapeLeft δ) := by
    congr 1
    dsimp [poly, coupled, Phase17.κX, Phase17.κY, argument, shapeLeft, truncatedSixthLowerAlpha]
    ring
  rw [he] at hH
  have hpoly := poly_mono coupled_nonneg (sub_nonneg.mpr ht.1)
    (show t-L0 ≤ t-shapeLeft δ by linarith only [hg.2.1])
  have hnum := mul_le_mul_of_nonneg_left hpoly (amplitude_nonneg hδ hδhi)
  have hp := mul_nonneg (amplitude_nonneg hδ hδhi) (poly_nonneg coupled_nonneg (sub_nonneg.mpr ht.1))
...
theorem actual_quad_lower {δ : ℝ} (hδ : 0 < δ) (hδhi : δ ≤ 1/100) :
    amplitude δ*(∫ t in L0..movingRight δ, f coupled t) ≤
  ...
  have hm := intervalIntegral.integral_mono_on hg.2.2.1.le
    ((f_integrable coupled le_rfl hg.2.2.1.le hg.2.2.2.1).const_mul (amplitude δ)) hi2
...
theorem three_piece_reciprocal_lower {δ : ℝ} (hδ : 0 < δ) (hδhi : δ ≤ 1/100) :
  ...
  change amplitude δ*(∫ t in L0..movingRight δ, f coupled t) ≤ ∫ t in shapeLeft δ..((1/2-δ)/2), kernel δ t at hlast
...
theorem actual_low_H_reciprocal_lower {δ : ℝ} (hδ : 0 < δ) (hδhi : δ ≤ 1/100) :
    movingC coupled δ*amplitude δ ≤ gainH34 δ/4 := by
  have hi := three_piece_reciprocal_lower hδ hδhi
  have he : movingC coupled δ*amplitude δ = Ctail*amplitude δ+Cmid*amplitude δ+
      2*(amplitude δ*(∫ t in L0..movingRight δ, f coupled t)) := by unfold movingC; ring
  rw [he,Ctail_identity,Cmid_identity]
  dsimp [gainH34,windowGain]
  linarith only [hi]

def g18 : ℝ := C18*sourceP/(1-rhoCoupled)
def B18 : ℝ := M18*sourceP/(1-rhoCoupled)+(100/49)*C18*envelopeE/(1-rhoCoupled)

theorem g18_pos : 0 < g18 :=
  div_pos (mul_pos C18_pos (by norm_num [sourceP])) (sub_pos.mpr rhoCoupled_bounds.2)

theorem B18_pos : 0 < B18 := by
  unfold B18
  exact add_pos (div_pos (mul_pos M18_pos (by norm_num [sourceP])) (sub_pos.mpr rhoCoupled_bounds.2))
    (div_pos (mul_pos (mul_pos (by norm_num) C18_pos) (by norm_num [envelopeE]))
      (sub_pos.mpr rhoCoupled_bounds.2))

/-- Positive moving coefficients pay the amplitude from below; no upper amplitude bound. -/
theorem gain18_linear_error {δ : ℝ} (hδ : 0 < δ) (hδhi : δ ≤ 1/100) :
    g18-B18*δ ≤ gainH34 δ/4 := by
  have hc := movingC_bounds coupled_nonneg hδ hδhi
  change 0 ≤ movingC coupled δ ∧ movingC coupled δ ≤ C18 ∧ C18-M18*δ ≤ movingC coupled δ at hc
  ...
  have hpenhi := mul_le_mul_of_nonneg_right (ParentPhase9HalfWeight.penalty_le hδ hδhi)
    (div_pos (mul_pos C18_pos he) hd).le
  have hrewrite : movingC coupled δ*((sourceP-penalty δ*envelopeE)/(1-rhoCoupled)) =
      movingC coupled δ*(sourceP/(1-rhoCoupled))-movingC coupled δ*(penalty δ*(envelopeE/(1-rhoCoupled))) := by ring
  rw [hrewrite] at hA
  unfold g18 B18
