...
theorem rectangle_integral {p : ℝ → ℝ} {a b c d K : ℝ}
  ...
  have hs' : Function.support (fun y => rawMasked (Icc a b ×ˢ Icc c d) p (x,y)) ⊆
    ...
    intro y hy
    by_contra hn
    exact hy (if_neg (fun hv => hn hv.2))
  dsimp only
...
theorem weighted_source_comparison {p : ℝ → ℝ} {w : Fin 21 → ℝ} {K : ℝ}
  ...
  unfold published regionA regionB
  linarith only [h]
...
theorem boundedH_eq {delta s : ℝ} (hs : s ∈ Icc 1 10) :
  ...
  by_cases h : s ≤ 2
  · rw [boundedH,if_pos h,min_eq_right h,max_eq_right hs.1]
  · rw [boundedH,if_neg h,min_eq_right hs.2,max_eq_right (le_of_not_ge h)]
...
theorem original_whole_source_lower {w : Fin 21 → ℝ} {delta : ℝ}
  ...
  apply weighted_source_comparison (boundedH_measurable hd hdhi)
    (wuImprovementLimit_nonneg false (s := 2) hd (by linarith) (by norm_num) (by norm_num))
    hb (fun v _ => (boundedH_bounds hd hdhi (u v.1 v.2)).1)
  intro v hv
