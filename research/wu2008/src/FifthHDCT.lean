...
noncomputable def fifthHEffectiveWeight (δ : ℝ) (n : ℕ) (j : ℕ × ℕ) : ℝ :=
  truncatedSixthClosureCoefficient true δ (truncatedSixthLowerS δ
    (truncatedSixthClosureHi n j.1) (truncatedSixthClosureHi n j.2))

noncomputable def fifthHDensity (δ : ℝ) (n : ℕ) (j : ℕ × ℕ) : ℝ :=
  fifthHEffectiveWeight δ n j /
...
noncomputable def fifthHStep (δ : ℝ) (n : ℕ) (v : ℝ × ℝ) : ℝ :=
  ∑ j ∈ fifthPairInner n,
    (truncatedSixthClosureCell n j).indicator (fun _ => fifthHDensity δ n j) v

noncomputable def fifthHKernel (δ : ℝ) (v : ℝ × ℝ) : ℝ :=
  if v ∈ fifthPairRegion then
    truncatedSixthClosureCoefficient true δ (truncatedSixthLowerS δ v.1 v.2) /
      (v.1 * v.2 * (truncatedSixthLowerC δ - v.1 - v.2)) else 0

theorem fifthH_density_bounds {δ : ℝ}
    (hδ : 0 < δ) (hδhi : δ ≤ 1 / 1000) (n : ℕ) (j : ℕ × ℕ) :
    0 ≤ fifthHDensity δ n j ∧
      fifthHDensity δ n j ≤ fifthPairKernelBound := by
  have hα := truncatedSixthLower_parameters.1
  have hβ := hα
  have hw := truncatedSixthClosure_coefficient_bounds hδ (by linarith) true
    (truncatedSixthLowerS δ (truncatedSixthClosureHi n j.1) (truncatedSixthClosureHi n j.2))
...
theorem fifthH_step_at_cell {δ : ℝ} {n : ℕ}
    {j : ℕ × ℕ} {v : ℝ × ℝ} (hj : j ∈ fifthPairInner n)
    (hv : v ∈ truncatedSixthClosureCell n j) :
    fifthHStep δ n v = fifthHDensity δ n j := by
  unfold fifthHStep
  rw [sum_eq_single j]
...
theorem fifthH_step_measurable (δ : ℝ) (n : ℕ) :
    Measurable (fifthHStep δ n) := by
  apply Finset.measurable_sum
  intro j _
  exact measurable_const.indicator (truncatedSixthClosure_cell_measurable n j)

theorem fifthH_step_bound {δ : ℝ}
  ...
  · obtain ⟨j, hj, hv⟩ := hex
    ...
    rw [fifthH_step_at_cell hj hv, Set.indicator_of_mem
      hunit']
    have hb := fifthH_density_bounds hδ hδhi n j
    simpa [Real.norm_eq_abs, abs_of_nonneg hb.1] using hb.2
  · rw [fifthH_step_zero (by simpa only [not_exists, not_and] using hex), norm_zero]
    exact Set.indicator_nonneg (fun _ _ => hK) _

theorem fifthH_density_tendsto {δ : ℝ} {v : ℝ × ℝ}
    (hδ : 0 < δ) (hδhi : δ ≤ 1 / 1000)
    (hv : v ∈ fifthPairRegion)
    (hc : ContinuousAt (truncatedSixthClosureCoefficient true δ) (truncatedSixthLowerS δ v.1 v.2)) :
    Tendsto (fun n => fifthHDensity δ n
      (truncatedSixthClosureIndex n v.1, truncatedSixthClosureIndex n v.2))
      atTop (𝓝 (fifthHKernel δ v)) := by
  ...
  have hw := hc.tendsto.comp (hscont.continuousAt.tendsto.comp hup)
  ...
  change Tendsto (fun n => fifthHDensity δ n (j n)) atTop
    (𝓝 (truncatedSixthClosureCoefficient true δ (truncatedSixthLowerS δ v.1 v.2) /
  ...
  rw [hde] at hlim
  simpa only [fifthHKernel, if_pos hv] using hlim

theorem fifthH_step_tendsto {δ : ℝ} {v : ℝ × ℝ}
    (hδ : 0 < δ) (hδhi : δ ≤ 1 / 1000)
    (hfront : v ∉ frontier (fifthPairRegion))
    (hc : ContinuousAt (truncatedSixthClosureCoefficient true δ) (truncatedSixthLowerS δ v.1 v.2)) :
    Tendsto (fun n => fifthHStep δ n v)
      atTop (𝓝 (fifthHKernel δ v)) := by
  ...
  · have hr := hv
    ...
    apply (fifthH_density_tendsto hδ hδhi hv hc).congr' 
    ...
    apply (fifthH_step_at_cell ?_ ?_).symm
  ...
  · simp only [fifthH_step_outside hv, fifthHKernel, if_neg hv]
    exact tendsto_const_nhds

theorem fifthH_integral_tendsto {δ : ℝ}
    (hδ : 0 < δ) (hδhi : δ ≤ 1 / 1000) :
    Tendsto (fun n => ∫ v : ℝ × ℝ, fifthHStep δ n v) atTop
      (𝓝 (∫ v : ℝ × ℝ, fifthHKernel δ v)) := by
  have hcoeff : ∀ᵐ v : ℝ × ℝ, ContinuousAt (truncatedSixthClosureCoefficient true δ)
