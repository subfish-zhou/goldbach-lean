...
theorem augmentedMatrix_le_feedbackMatrix (i k : Fin 9) :
  ...
  (add_le_add (oldMatrix_le_elementary i k) (le_refl (extraMatrix i k))).trans
...
theorem actual_augmented_system {δ : ℝ} (hd : 0 < δ) (hh : δ ≤ 1 / 10)
  ...
  exact (add_le_add (le_refl _) hm).trans ha
...
theorem augmented_same_delta :
  ...
  exact (add_le_add (le_refl _)
    (augmented_apply_le_feedback (actualNine_nonneg hδ (hr.trans hcap)) i)).trans
      (hs δ hδ hr i)
