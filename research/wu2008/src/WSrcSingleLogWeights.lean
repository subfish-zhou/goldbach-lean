...
theorem source_kernel_integral {a b : ℝ} (ha : 0 < a) (hab : a ≤ b)
  ...
  have hderiv : ∀ s ∈ uIcc a b, HasDerivAt F (sourceKernel s) s := by
    ...
    have hval : sourceKernel s =
        1 / s - -(2 * truncatedSixthLowerAlpha) / (1 - 2 * truncatedSixthLowerAlpha * s) := by
      rw [div_sub_div _ _ hs0 hd]
      unfold sourceKernel
      congr 1
      ring
    rw [hval]
    convert h using 1 <;> first | rfl | simp only [mul_one, id_eq]
...
theorem fourth_grid_mono {k : ℕ} (hk : k < 16) :
  ...
  · subst k
    norm_num [fourthGrid]
  · simp only [fourthGrid, if_neg hz, Nat.succ_ne_zero,
      if_false, Nat.cast_add, Nat.cast_one]
    linarith
...
theorem fourth_cell_lower {δ z : ℝ} {k : ℕ}
  ...
  have hi := ((source_kernel_continuous ha0 hab hb.2).intervalIntegrable
    (μ := volume)).const_mul (2 * z)
  ...
  have hm := intervalIntegral.integral_mono_on hab hi hH (fun s hs => by
    ...
    have hpos0 : 0 < s * (1 - 2 * truncatedSixthLowerAlpha * s) := by
      ...
      norm_num [truncatedSixthLowerAlpha]
      linarith
    have harg : s ≤ sourceNode (14 + k) := by
      rw [← fourth_grid_source_node]
      exact hs.2
...
theorem fourth_nodes_to_source {δ : ℝ} {H : ℕ → ℝ}
  ...
  have hlow : (∑ k ∈ range 16,
    ...
    have hidx : 14 + k ∈ Finset.Icc 14 29 := Finset.mem_Icc.mpr ⟨by omega, by omega⟩
    exact fourth_cell_lower hd hh hk16 (hH0 _ hidx) (hH _ hidx)
  have htrunc := fourth_truncated_le_source hd hh
  rw [show fourthGrid 0 = (1327 / 400 : ℝ) by norm_num [fourthGrid],
    show fourthGrid 16 = (49 / 10 : ℝ) by norm_num [fourthGrid] ] at hlow
  simp_rw [mul_assoc] at hlow
  rw [← mul_sum] at hlow
  unfold fourthNodeSum
  linarith only [hlow, htrunc]
