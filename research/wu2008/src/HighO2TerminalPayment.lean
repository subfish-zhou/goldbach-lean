...
/-- The one complete AP error is paid against actual original Theta.
The genuine reciprocal convolution mass prevents a cell-count loss. -/
theorem original_AP_relative {δ ε : ℝ}
  ...
  obtain ⟨C,hC,T0,hT04,hAP⟩ := original_AP_paid hδ hδhi
  ...
  calc
    _ ≤ C*N/log (N : ℝ)^(18 : ℝ) := hAP N hN0 Δ hlo hhi V hV hr
