...
theorem fullWeight_formula {n : ℕ} (j : Fin n) (phi : ℝ) (t : Fin n → ℝ) :
  ...
  simp [fullWeight, geometricWeight, continuousDensity, div_eq_mul_inv,
    Finset.prod_inv_distrib, mul_comm, mul_assoc]
...
theorem fullWeight_three (phi : ℝ) (t : Fin 3 → ℝ) :
  ...
  change buchstab ((phi - (t 0 + (t 1 + t 2))) / t 1) /
    (t 1 * (t 0 * (t 1 * t 2))) = _
  congr 1
  · congr 1
    ring
...
theorem fullWeight_four (phi : ℝ) (t : Fin 4 → ℝ) :
  ...
  simp only [Fin.sum_univ_succ, Fin.sum_univ_zero, add_zero,
    Fin.prod_univ_succ, Fin.prod_univ_zero, mul_one]
  change buchstab ((phi - (t 0 + (t 1 + (t 2 + t 3)))) / t 2) /
    (t 2 * (t 0 * (t 1 * (t 2 * t 3)))) = _
  congr 1
  · congr 1
    ring
  · ring
...
theorem fullWeight_five (phi : ℝ) (t : Fin 5 → ℝ) :
  ...
  simp only [Fin.sum_univ_succ, Fin.sum_univ_zero, add_zero,
    Fin.prod_univ_succ, Fin.prod_univ_zero, mul_one]
  change buchstab ((phi - (t 0 + (t 1 + (t 2 + (t 3 + t 4))))) / t 3) /
    (t 3 * (t 0 * (t 1 * (t 2 * (t 3 * t 4))))) = _
  congr 1
  · congr 1
    ring
  · ring
...
theorem fullWeight_six (phi : ℝ) (t : Fin 6 → ℝ) :
  ...
  simp only [Fin.sum_univ_succ, Fin.sum_univ_zero, add_zero,
    Fin.prod_univ_succ, Fin.prod_univ_zero, mul_one]
  change buchstab ((phi - (t 0 + (t 1 + (t 2 + (t 3 + (t 4 + t 5)))))) / t 4) /
    (t 4 * (t 0 * (t 1 * (t 2 * (t 3 * (t 4 * t 5)))))) = _
  congr 1
  · congr 1
    ring
  · ring
...
theorem fullWeight_split {n : ℕ} (j : Fin n) (phi : ℝ)
  ...
  by_cases hl : t ∈ HighNonunitLegal.legal j phi
  · rw [Set.indicator_of_notMem (show t ∉ (HighNonunitLegal.legal j phi)ᶜ by
      simpa only [Set.mem_compl_iff, not_not] using hl),
...
theorem lowerK_eq (p : SecondFunctionalParameters) (hp : p.MotherAdmissible)
  ...
  change fullWeight 1 phi t = HighNonunitLegal.G 1 phi t * continuousDensity t
