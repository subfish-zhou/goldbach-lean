import WE09JointMainMajorMiddlePayment
...
theorem log_lower_quadratic {y : ℝ} (hy : 0 ≤ y) :
  ...
  have hm : MonotoneOn F (Ici 0) := by
    ...
    · intro x hx
      have hh : 0 ≤ x := interior_subset hx
      rw [(hd x hh).deriv]
...
theorem log_three_cubic {y : ℝ} (hy : 0 ≤ y) :
  ...
  have hd (x : ℝ) (hx : 0 ≤ x) : HasDerivAt F (x ^ 3 / (27 * (3 + x))) x := by
    ...
    have h1 := (hasDerivAt_const x (log (3 : ℝ))).add ((hasDerivAt_id x).div_const 3)
    have h2 := h1.sub (((hasDerivAt_id x).pow 2).div_const 18)
    have hp := h2.add (((hasDerivAt_id x).pow 3).div_const 81)
    convert! hp.sub (((hasDerivAt_id x).const_add 3).log hn) using 1
    dsimp
    field_simp [hn]
    ring
  have hm : MonotoneOn F (Ici 0) := by
...
theorem upper_delayed_high {v : ℝ} (hv : 5 ≤ v) (hv6 : v ≤ 6) :
  ...
  · have hd (x : ℝ) : HasDerivAt (fun x => highIncrement (x - 4)) (highDensity (x - 4)) x := by
      convert! (highIncrement_derivative (x - 4)).comp x ((hasDerivAt_id x).sub_const 4) using 1
      simp
