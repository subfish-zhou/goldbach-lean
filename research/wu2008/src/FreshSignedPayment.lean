...
/-- A single, fully signed payment of the collected eight-kernel expression. -/
def lower : ℝ :=
  coeffTwo*twoUpper+coeffThreeHalf*splitHigh (3/2)+splitLow (1127/600)+
  coeffA*splitLow (3884129/3606400)+coeffAC*splitHigh (4508/2927)-
  coeffB*splitLow (2400/2381)+coeffD*splitHigh (4508/3981)+coeffS*splitLow (2254/1727)+
  poleZero*splitHigh (927/800)+poleOne*splitLow (727/600)+
  coeffAM*splitLow crossOneMinus+coeffAP*splitLow crossOnePlus+
  coeffTM*splitHigh crossTwoMinus+coeffTP*splitLow crossTwoPlus+
  coeffBM*splitLow bCrossOneMinus+coeffBP*splitLow bCrossOnePlus+
  coeffDM*splitHigh bCrossTwoMinus+coeffDP*splitLow bCrossTwoPlus+rational+EJoint.payment
...
theorem lower_le_collected : lower ≤ collected := by
  ...
  have h16 := mul_le_mul_of_nonpos_left (le_splitHigh hbym) coeffDM_sign
