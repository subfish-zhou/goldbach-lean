...
theorem prime_integral_bounds (j : Fin 3) {δ : ℝ} (hh : δ ≤ 1/100) :
  ...
  · have h := intervalIntegral.integral_mono_on hg.2.1.le hi
    ...
    change primeIntegral j δ ≤ _ at h
...
theorem theta_exact {N : ℕ} {δ : ℝ} (j : Fin 3) (hN : 2 ≤ N) (he : Even N) :
  ...
  change boxTheta N ((N : ℝ)^(1/2-δ))
    (fun _ : Fin 1 => psiPrimes (j.castAdd 4) N) = _
  rw [SingleUpperNormalization.theta_single_exact hN
  ...
  have hF : ContinuousAt F 0 := by
    ...
    · apply ContinuousAt.log
      · apply ContinuousAt.div
        · fun_prop
        · fun_prop
        · simpa only [sub_zero] using (mul_pos ha hcb).ne'
      · simpa only [sub_zero] using (div_pos (mul_pos hb hca) (mul_pos ha hcb)).ne'
    · fun_prop
    · norm_num
  apply hF.congr_of_eventuallyEq
  filter_upwards [eventually_lt_nhds (show (0 : ℝ) < 1/100 by norm_num)] with δ hδ
  exact prime_integral_literal j hδ.le
