...
theorem fifth_denominator_upper {v : ℝ × ℝ} (hv : v ∈ fifthPairRegion) :
  ...
  simp only [fifthDenominator, truncatedSixthLowerC, sub_zero]
  calc
    _ ≤ b*b*(1/2-b-b) := h
    _ = b^2*(1/2-2*b) := by ring
...
theorem fifth_gain_lower : fifthGainLower ≤ PositiveCoreResume.fifthGain := by
  ...
  calc
    _ = 4*(fifthSeed/fifthDenominator*
        (truncatedSixthLowerBeta-truncatedSixthLowerAlpha)^2/2) := by
      dsimp only [a, b]
      ring
    _ ≤ _ := mul_le_mul_of_nonneg_left hm (by norm_num)
