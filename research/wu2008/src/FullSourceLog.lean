...
def movingKernel (δ : ℝ) (v : ℝ × ℝ) : ℝ :=
  if truncatedSixthLowerAdmissibleRegion δ v.1 v.2 then
    profile (truncatedSixthLowerS δ v.1 v.2) /
      (v.1*v.2*(truncatedSixthLowerC δ-v.1-v.2)) else 0

/-- A uniform payment kernel. Only the original endpoint d controls the mask;
...
theorem moving_nonneg {δ : ℝ} (hδ : 0 ≤ δ) (v : ℝ × ℝ) :
  ...
  unfold movingKernel
  split_ifs with hv
  · exact div_nonneg (profile_nonneg _) (denominator_pos hδ hv).le
  · exact le_rfl
...
theorem moving_lower {δ : ℝ} (hδ : 0 < δ) (hδhi : δ ≤ 1/1000)
  ...
  · have hb := truncatedSixthLower_region_bounds hδ.le hv.1
    exact div_le_div_of_nonneg_right (profile_lower hδ hδhi hb.2.2.2.1 hb.2.2.2.2)
...
theorem uniform_nonneg {d : ℝ} (hd : 0 ≤ d) (v : ℝ × ℝ) :
  ...
  split_ifs with hv
  · exact div_nonneg (profile_nonneg _) (denominator_pos le_rfl (region_mono hd hv)).le
  · exact le_rfl
...
theorem uniform_le_moving {δ d : ℝ} (hδ : 0 ≤ δ) (hδd : δ ≤ d)
  ...
  · have hvδ := region_mono hδd hv
    ...
    have hz : v.1*v.2*(truncatedSixthLowerC δ-v.1-v.2) ≤
      ...
      linarith
    have hb0 := truncatedSixthLower_region_bounds (δ := 0) le_rfl hv0.1
    have hmono := profile_antitone ⟨hb.2.2.2.1,hb.2.2.2.2⟩
      ⟨hb0.2.2.2.1,hb0.2.2.2.2⟩ hs
    exact div_le_div₀ (profile_nonneg _) hmono (denominator_pos hδ hvδ) hz
  · rw [uniformKernel, if_neg hv]
    exact moving_nonneg hδ v

theorem moving_measurable (δ : ℝ) : Measurable (movingKernel δ) := by
  have hs : Measurable (fun v : ℝ × ℝ => profile (truncatedSixthLowerS δ v.1 v.2)) := by
    unfold profile truncatedSixthLowerS
    fun_prop
...
theorem uniform_measurable (d : ℝ) : Measurable (uniformKernel d) := by
  have hs : Measurable (fun v : ℝ × ℝ => profile (truncatedSixthLowerS 0 v.1 v.2)) := by
    unfold profile truncatedSixthLowerS
    fun_prop
...
/-- Full moving full-source logarithmic-profile payment, with no discarded subrectangle complement. -/
...
/-- Uniform in every positive delta below d; only the explicit log kernel is frozen. -/
...
theorem gammaLog6_payment {δ : ℝ} (hδ : 0 < δ) (hδhi : δ ≤ 1/1000) :
    GammaLog6 ≤ truncatedSixthLowerHadmdelta δ := uniform_payment hδ hδhi le_rfl
...
theorem actual_count_lower {ε : ℝ} (hε : 0 < ε) :
    ∃ T : ℕ, 4 ≤ T ∧ ∀ N : ℕ, T ≤ N → Even N →
      (truncatedSixthLowerF6lin+GammaLog6-ε)*wuSingularSeries N*N/log N^(2 : ℕ) ≤
      (truncatedSixthMass N ((N : ℝ)^truncatedSixthLowerAlpha)
        ((N : ℝ)^truncatedSixthLowerBeta) ((N : ℝ)^truncatedSixthLowerSigma)
        ((N : ℝ)^truncatedSixthLowerLambda) : ℝ) := by
