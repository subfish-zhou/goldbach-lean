...
theorem actual_single_count {ε : ℝ} (hε : 0 < ε) :
  ...
  have hn : ∀ i ∈ Finset.Icc 14 29, 0 ≤ SourceNodeInstance.H i := by
    intro i hi
    exact SourceNodeInstance.upper_nonneg (by have := (Finset.mem_Icc.mp hi).1; omega)
      (Finset.mem_Icc.mp hi).2
  have hs : ∀ δ : ℝ, 0 < δ → δ ≤ WuSource.SrcNine.d → δ ≤ 1/100 →
      SourceNodeInstance.h 22 ≤ wuImprovementLimit false δ (21/5) ∧
      (∀ t ∈ Icc (78/25 : ℝ) (16/5), SourceNodeInstance.H 12 ≤
        wuImprovementLimit true δ t) ∧
      (∀ i ∈ Finset.Icc 14 29, SourceNodeInstance.H i ≤
        wuImprovementLimit true δ (WuSource.SrcSingle.sourceNode i)) := by
    intro δ hδ hr _
    have htable := SourceNodeInstance.actual_tables hδ hr
    refine ⟨?_, ?_, ?_⟩
    · simpa only [show rNode 22 = (21/5 : ℝ) by norm_num [rNode]] using
        htable.2 22 (by norm_num)
    · intro t ht
  ...
  simpa only [amount, add_assoc] using
    (WuSource.SrcSingle.original_Hh_input_to_count WuSource.SrcNine.d_pos
      small_profile_integrable hn hs hε)
